...
function resolveEpisodeBadge(nextEpAirDate: string | null, daysSince: number | null): string | null {
    ...
    const days = refDate
        ? Math.max(0, Math.floor((Date.now() - new Date(refDate).getTime()) / 86400000))
        : daysSince;
    if (days === null)
        return "NOVO"; // sem data → assume novo para não suprimir
    if (days <= NEW_EPISODE_BADGE_DAYS)
        return "NOVO";
    return uiMessage("ui.3ec461caa9d1");
}
export default function NewEpisodeCard({ item, onClick }: Props) {
    ...
    const seasonPct = item.season_total != null && item.season_total > 0 && item.season_watched != null
    ...
    return (<button type="button" onClick={onClick} className="group relative w-full overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-900/80 text-left transition-all hover:border-white/[0.16] hover:bg-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30">
      {backdropUrl && (<div className="absolute inset-0 -z-0">
          <Image src={backdropUrl} alt="" fill unoptimized sizes="(max-width: 768px) 100vw, 50vw" className="h-full w-full object-cover object-center opacity-35 brightness-75 transition-opacity group-hover:opacity-45"/>
      ...
      <div className="relative flex min-h-[72px] items-center gap-3 px-4 py-3.5 sm:gap-4 sm:px-5 sm:py-4">
        {posterUrl && (<div className="relative hidden shrink-0 overflow-hidden rounded-lg border border-white/[0.10] sm:block" style={{ width: 40, height: 58 }}>
            <Image src={posterUrl} alt="" fill unoptimized sizes="40px" className="h-full w-full object-cover"/>
          </div>)}

        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
            {newEpisodeBadge && (<span className={[
            ...
            {epTag && (<span className="rounded-full border border-white/[0.15] bg-white/[0.07] px-2 py-0.5 text-[10px] font-bold text-white/75">
                {epTag}
              </span>)}
          ...
          {item.next_episode_name && (<p className="mt-0.5 truncate text-[12px] leading-tight text-white/50">{uiMessage("ui.7458eb22633c")}{item.next_episode_name}{uiMessage("ui.55f8f15fa29d")}</p>)}
        </div>

        <div className="flex shrink-0 items-center gap-2.5">
          {item.runtime_label && (<span className="text-[12px] font-semibold tabular-nums text-white/45">
              {item.runtime_label}
            </span>)}

          <span aria-hidden className="flex h-7 w-7 items-center justify-center rounded-full bg-white/[0.07] text-white/60 transition-all group-hover:bg-rose-500/80 group-hover:text-white">
      ...
      <div className="relative h-[3px] w-full overflow-hidden bg-white/[0.05]">
        {progressPct > 0 && (<div className="absolute inset-y-0 left-0 bg-gradient-to-r from-rose-400 via-pink-400 to-fuchsia-400" style={{ width: `${progressPct}%` }}/>)}
      </div>
    </button>);
}
