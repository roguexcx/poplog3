...
export type ContinueStatusSignal = "new_episode" | "last_episode" | "reta_final" | "continuing";
...
const SIGNAL_CONFIG: Record<ContinueStatusSignal, {
    label: string | null;
    color: string;
    bar: string;
}> = {
    new_episode: {
        label: uiMessage("ui.e45e696ac0c5"),
        color: "text-rose-300/90",
        bar: "from-rose-400 via-pink-400 to-fuchsia-400",
    },
    last_episode: {
        label: uiMessage("ui.034f9bdadd28"),
...
export default function ContinueCard({ item, onClick }: Props) {
    ...
    const backdropUrl = resolveCatalogImage(item.next_episode_still_path, "w780") ??
        resolveCatalogImage(item.backdrop_path, "w780");
    const cfg = SIGNAL_CONFIG[item.status_signal];
    const seasonPct = item.season_total != null && item.season_total > 0
    ...
    const statusLabel = item.status_signal === "reta_final"
    ...
    const catchUpRuntimeLabel = formatCatchUpLabel(item.series_remaining_runtime_label ?? item.remaining_runtime_label);
    const seasonProgressLabel = item.season_total != null
        ? `${item.season_watched} de ${item.season_total} eps na T${item.next_season}`
        : null;
    return (<button type="button" onClick={onClick} className="group relative w-full overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-900/80 text-left transition-all hover:border-white/[0.16] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30">
      {/* Backdrop (right side) */}
      {backdropUrl && (<div className="absolute inset-0 -z-0">
          <img src={backdropUrl} alt="" className="h-full w-full object-cover object-center opacity-25 brightness-75 transition-opacity group-hover:opacity-35"/>
      ...
      <div className="relative flex min-h-[92px] items-stretch gap-0">
        {/* Poster */}
        {posterUrl && (<div className="relative shrink-0 self-stretch overflow-hidden rounded-l-2xl" style={{ width: 62 }}>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between px-3.5 py-3 sm:px-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {isNewEp && (<span className="inline-flex items-center rounded-full bg-rose-500 px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.12em] text-white">{uiMessage("ui.512ae76d872e")}</span>)}
            {!posterUrl && (<span className="rounded-full border border-white/[0.15] bg-white/[0.07] px-2 py-0.5 text-[10px] font-bold text-white/70">
                T{item.next_season}E{item.next_episode}
              </span>)}
          ...
          <div className="mt-1 min-w-0">
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              {catchUpRuntimeLabel && (<span title={catchUpRuntimeLabel} className="max-w-[48%] shrink-0 truncate rounded-full border border-white/[0.10] bg-white/[0.055] px-1.5 py-px text-[9px] font-bold leading-tight text-white/50">
                  {catchUpRuntimeLabel}
                </span>)}
            </div>
            {item.next_episode_name && (<p className="mt-0.5 truncate text-[11px] leading-tight text-white/48">{uiMessage("ui.7458eb22633c")}{item.next_episode_name}{uiMessage("ui.55f8f15fa29d")}</p>)}
            {(seasonProgressLabel || activeRuntimeLabel) && (<p className="mt-0.5 flex items-center gap-1 truncate text-[10px] text-white/35">
                {seasonProgressLabel && (<span className="shrink-0">{seasonProgressLabel}</span>)}
                {activeRuntimeLabel && (<span className="inline-flex shrink-0 items-center rounded-full px-1.5 py-px">
          ...
          <div className="mt-1.5 flex items-center justify-between gap-2">
            {statusLabel ? (<p className={`truncate text-[11px] font-semibold leading-none ${cfg.color}`}>
                {statusLabel}
              </p>) : (<span />)}
            {progressPct > 0 && (<span className="shrink-0 text-[11px] font-bold tabular-nums text-white/40">
                {progressPct}%
              </span>)}
          </div>
        </div>
      ...
      <div className="relative h-[3px] w-full overflow-hidden bg-white/[0.05]">
        {progressPct > 0 && (<div className={`absolute inset-y-0 left-0 bg-gradient-to-r ${cfg.bar}`} style={{ width: `${progressPct}%` }}/>)}
      </div>
    </button>);
}
