...
function ProviderBadge({ name, logoPath, type, }: {
    name: string | null;
    ...
    return (<div className="flex items-center gap-1 rounded-md bg-black/60 px-1.5 py-0.5 backdrop-blur-sm">
      {logoPath ? (<img src={resolveCatalogImage(logoPath, "original") ?? ""} alt={name} className="h-3 w-3 rounded-sm object-contain"/>) : null}
      <span className={[
...
export default function WatchlistPickCard({ item, onClick }: Props) {
    const posterUrl = resolveCatalogImage(item.poster_path, "w342") ??
    ...
    const subtitle = (() => {
        if (!isMovie) {
            if (item.number_of_episodes) {
                const seasons = item.number_of_seasons && item.number_of_seasons > 1
                    ? ` · ${item.number_of_seasons} temp`
                    : "";
                return `${item.number_of_episodes} ep${seasons}`;
            }
            return item.release_year ? String(item.release_year) : uiMessage("ui.74f19285073a");
        }
        return item.release_year ? String(item.release_year) : "Filme";
    })();
    return (<button type="button" onClick={onClick} className="group relative w-full text-left">
      {/* Poster */}
      <div className="relative mb-2.5 aspect-[2/3] overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.03] transition-shadow duration-300 group-hover:shadow-[0_14px_40px_rgba(0,0,0,0.45)]">
        {posterUrl ? (<img src={posterUrl} alt={item.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.04]" loading="lazy"/>) : (<div className="flex h-full w-full items-center justify-center">
        ...
        <div className="absolute left-2 right-2 top-2 flex items-start justify-between gap-1">
          ...
          {item.vote_average != null && item.vote_average > 0 && (<span className="flex items-center gap-0.5 text-[9px] font-bold text-amber-300">
        ...
        {item.best_provider_name && (<div className="absolute bottom-2 left-2 right-2">
            <ProviderBadge name={item.best_provider_name} logoPath={item.best_provider_logo} type={item.best_provider_type}/>
          </div>)}
      </div>

      {/* Title + subtitle */}
      <div className="px-0.5">
        <LocalizedTitle as="div" title={item.title} originalTitle={item.original_title} variant="poster" className="mb-0.5 line-clamp-2 min-h-[2.15rem] text-[12px] font-semibold leading-tight text-white/85 transition-colors group-hover:text-white"/>
