...
export default function LibraryPosterCard({ item, priority, inCustomList = false, }: LibraryPosterCardProps) {
    const title = item.title;
    const isFavorite = item.favorite === true;
    const isWatching = item.media_type === "movie"
      ...
      <article className="relative">
        ...
        <div className={[
          ...
          <div className="relative aspect-[2/3] overflow-hidden bg-white/[0.04]">
            <TmdbImage path={title?.poster_path ?? null} fallbackPath={title?.backdrop_path ?? null} size="w500" alt={displayTitle} fallbackLabel={displayTitle} priority={priority} className="h-full w-full object-cover brightness-[0.88] saturate-[1.08] transition duration-500 group-hover:scale-[1.06] group-hover:brightness-[1.04]"/>
            ...
            <div className={`absolute left-2 top-2 rounded-full border px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.12em] shadow-[0_8px_22px_rgba(0,0,0,0.38)] backdrop-blur-md sm:left-3 sm:top-3 sm:px-2.5 sm:py-1 sm:text-[9px] sm:tracking-[0.14em] ${STATUS_BADGE[item.status] ?? STATUS_BADGE.watched}`}>
              {formatStatus(item)}
            </div>

            <div className="absolute right-2 top-2 flex flex-col items-end gap-1.5 sm:right-3 sm:top-3">
              {isFavorite ? (<div className="rounded-full border border-amber-300/30 bg-amber-500/22 px-2 py-0.5 text-[9px] font-black text-amber-200 shadow-[0_0_16px_rgba(251,191,36,0.30)] backdrop-blur-md sm:py-1">
                  ★
                </div>) : typeof title?.vote_average === "number" ? (<div className="rounded-full border border-amber-200/18 bg-black/50 px-2 py-0.5 text-[9px] font-black text-amber-100 shadow-[0_8px_22px_rgba(0,0,0,0.35)] backdrop-blur-md sm:px-2.5 sm:py-1 sm:text-[10px]">
                  ★ {title.vote_average.toFixed(1)}
                </div>) : null}
            </div>

            {inCustomList && (<span className="absolute bottom-2 left-2 grid h-7 w-7 place-items-center rounded-full border border-indigo-200/25 bg-indigo-950/72 text-indigo-100 shadow-[0_8px_22px_rgba(0,0,0,0.42)] backdrop-blur-md sm:bottom-3 sm:left-3" title={uiMessage("ui.1dc929406e22")} aria-label={uiMessage("ui.1dc929406e22")}>
                <Layers3 className="h-3.5 w-3.5" aria-hidden/>
              </span>)}

            {hasProvider && (<div className="absolute bottom-2 right-2 overflow-hidden rounded-md border border-white/[0.14] bg-black/55 shadow-[0_4px_14px_rgba(0,0,0,0.45)] backdrop-blur-md sm:bottom-3 sm:right-3 sm:rounded-lg">
                {providerLogoSrc ? (<Image src={providerLogoSrc} alt={providerName ?? "Provider"} width={28} height={28} unoptimized className="h-6 w-6 object-cover sm:h-7 sm:w-7"/>) : (<span className="block max-w-[88px] truncate px-1.5 py-1 text-[8px] font-black uppercase tracking-[0.06em] text-white/85 sm:text-[9px]">
                    {providerName}
                  </span>)}
              </div>)}
            ...
            <div className="absolute inset-x-0 bottom-0 p-2.5 sm:p-4">
              <div className="translate-y-2 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                <div className="flex flex-wrap items-center gap-1.5">
                  {subtitle && (<span className="rounded-full border border-white/[0.12] bg-black/50 px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.12em] text-white/76 backdrop-blur-md sm:px-2.5 sm:py-1 sm:text-[9px] sm:tracking-[0.14em]">
                      {subtitle}
                    </span>)}

                  {runtimeLabels.map((runtime) => (<span key={runtime} className="rounded-full border border-white/[0.12] bg-black/50 px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.12em] text-white/55 backdrop-blur-md sm:px-2.5 sm:py-1 sm:text-[9px] sm:tracking-[0.14em]">
                      {runtime}
                    </span>))}
                </div>
              </div>
            ...
            {isWatching && progress !== null && progress > 0 && (<div className="absolute inset-x-0 bottom-0 h-[3px] bg-black/40">
          ...
          <div className="flex h-7 items-center px-2 sm:px-[10px]">
            <div className="flex items-center gap-[5px] text-[11px] font-normal leading-none text-white/40">
              ...
              <span>{item.media_type === "movie" ? "Filme" : uiMessage("ui.74f19285073a")}</span>
            </div>
          </div>
        </div>
      </article>
...
function getPosterSubtitle(item: Poplog3UserLibraryItem) {
    ...
    if (comingSoon.isComingSoon) {
        ...
        if (comingSoon.phase === "critical_recheck")
            return "Revalidando VOD";
        return "Em breve";
    }
...
function getRuntimeLabels(item: Poplog3UserLibraryItem) {
    if (item.media_type === "tv") {
        ...
        return fallback ? [fallback] : [uiMessage("ui.39fda122f238")];
    }
    if (item.runtime_label)
        return [item.runtime_label];
...
function formatStatus(item: Poplog3UserLibraryItem) {
    if (item.media_type === "tv" && item.computed_state === "up_to_date")
        return "Em dia";
    if (item.media_type === "tv" && item.computed_state === "completed")
        return uiMessage("ui.c384beb215bf");
