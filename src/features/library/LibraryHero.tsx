...
export default function LibraryHero({ library, totalCount }: LibraryHeroProps) {
    const heroItem = library.find((item) => item.title?.backdrop_path) ??
    ...
    return (<section className="relative w-full">
      <div className="relative min-h-[230px] overflow-hidden rounded-[1.65rem] border border-white/[0.06] bg-[#03040a] shadow-[0_32px_120px_rgba(0,0,0,0.58)] sm:min-h-[300px] sm:rounded-[2rem] md:min-h-[340px] lg:rounded-[2.5rem]">
        <div className="absolute inset-0">
          {heroItem?.title && (<TmdbImage path={heroItem.title.backdrop_path ?? heroItem.title.poster_path} fallbackPath={heroItem.title.poster_path ?? null} size="w1280" alt={heroTitle} fallbackLabel={heroTitle} className="h-full w-full object-cover opacity-[0.18] blur-[2px] scale-105 saturate-[0.85]"/>)}
        ...
        <div className="relative flex h-full min-h-[230px] flex-col justify-end p-5 sm:min-h-[300px] sm:p-8 md:min-h-[340px] md:p-10 lg:p-12">
          <div className="mb-3 flex items-center gap-3 sm:mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.28em] text-cyan-100/65">{uiMessage("ui.5138733bd6ca")}</span>
          </div>

          <h1 className="text-[42px] font-black leading-[0.9] tracking-[-0.04em] text-white sm:text-7xl sm:tracking-[-0.07em] md:text-8xl lg:text-[7rem]">{uiMessage("ui.497ac5c11bf3")}</h1>

          <p className="mt-3 max-w-xl text-[13px] leading-relaxed text-white/42 sm:mt-4 sm:text-[15px]">
            {totalCount > 0
            ? uiMessage("ui.a688e65bc4c7", { v1: totalCount, v2: totalCount === 1 ? uiMessage("ui.3b6b38a966cd") : uiMessage("ui.29b552496571") }) : uiMessage("ui.82dea273127d")}
          </p>
        </div>
      </div>
    </section>);
