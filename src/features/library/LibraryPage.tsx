...
import { Bookmark, Heart, PlayCircle, Calendar, CheckCircle2, Pause, X, Filter, ChevronLeft, ChevronRight, Search, } from "lucide-react";
...
import { resolveLibraryPopularityScore, deserializeTraktScoreMap, type TraktScoreMap, } from "@/lib/score/library-popularity";
...
const MEDIA_OPTIONS: {
    ...
    { id: "tv", label: uiMessage("ui.de212174bc0c") },
];
const SORT_OPTIONS: {
    value: SortBy;
    label: string;
}[] = [
    { value: "random", label: uiMessage("ui.177cecc372f8") },
    { value: "popularity-desc", label: "Popularidade" },
    { value: "recent", label: uiMessage("ui.edb5ca21e20b") },
    { value: "release-desc", label: uiMessage("ui.ef3c67f32bf6") },
    { value: "release-asc", label: uiMessage("ui.30049e653bb1") },
    { value: "title-asc", label: uiMessage("ui.e5e0e8ff7b94") },
    { value: "runtime-desc", label: uiMessage("ui.798c30eb2104") },
    { value: "runtime-asc", label: uiMessage("ui.68fafc187fe6") },
];
const TAB_OPTIONS: {
    id: LibraryTab;
    label: string;
    ...
    { id: "all", label: "Tudo" },
    { id: "random", label: uiMessage("ui.177cecc372f8") },
    { id: "watchlist", label: "Watchlist" },
    { id: "favorites", label: uiMessage("ui.33b332595fe4") },
    { id: "watching", label: "Maratonando" },
    { id: "coming-soon", label: uiMessage("ui.8a6ec37c6474") },
    { id: "watched", label: uiMessage("ui.454ea6968d53") },
...
const STATUS_LABEL: Record<string, string> = {
    ...
    watched: uiMessage("ui.ab53b20a28e7"),
...
function itemMatchesSearch(item: Poplog3UserLibraryItem, normalizedQuery: string): boolean {
    ...
    const haystack = normalizeSearchText([displayTitle, item.title?.title, item.title?.original_title, item.title?.year]
        .filter(Boolean)
        .join(" "));
...
export default function LibraryPage({ library, initialTab, traktScores, initialLists = [], listMembership = {}, isAuthenticated = false, }: LibraryPageProps) {
    ...
    const traktScoreMap = useMemo<TraktScoreMap>(() => (traktScores ? deserializeTraktScoreMap(traktScores) : new Map()), 
    ...
    const spotlightItem = useMemo<Poplog3UserLibraryItem | null>(() => {
        ...
        const seed = library.reduce((acc, item) => {
            ...
            return (acc + id * 31 + (item.media_type?.length ?? 0) * 17) % 1000003;
        }, library.length * 97);
    ...
    useEffect(() => {
        if (spotlightItem === null)
            console.warn("[spotlight] null — candidates vazio");
        else if (spotlightItem === undefined)
            console.warn("[spotlight] undefined — useMemo sem return");
        else
            console.log("[spotlight] ok —", spotlightItem.title?.title ?? spotlightItem.title?.original_title ?? `tmdb:${spotlightItem.tmdb_id}`);
    }, [spotlightItem]);
    // Editorial rails
    const watchlistItems = useMemo(() => library
        .filter((i) => isPureWatchlist(i) && !isComingSoon(i))
        .sort((a, b) => getPopularity(b, traktScoreMap) - getPopularity(a, traktScoreMap))
        .slice(0, 12), [library, traktScoreMap]);
    const recentItems = useMemo(() => library
    ...
    const favoritesItems = useMemo(() => library
        .filter((i) => i.favorite === true && !isComingSoon(i))
        .sort((a, b) => getPopularity(b, traktScoreMap) - getPopularity(a, traktScoreMap))
        .slice(0, 10), [library, traktScoreMap]);
    const comingSoonItems = useMemo(() => library
        .filter(isComingSoon)
    ...
    const filteredLibrary = useMemo(() => {
        ...
        if (yearFilter !== null)
            items = items.filter((i) => i.title?.year === yearFilter);
        const normalizedQuery = normalizeSearchText(searchQuery);
        if (normalizedQuery)
            items = items.filter((i) => itemMatchesSearch(i, normalizedQuery));
    ...
    return (<div className="relative flex flex-col gap-8 pb-20 md:gap-14 md:pb-0">
      ...
      {(favoritesItems.length > 0 || isAuthenticated) && (<section className={`relative grid gap-6 ${favoritesItems.length > 0 && isAuthenticated ? "lg:grid-cols-2 lg:gap-6" : ""}`}>
          {/* Favoritos — esquerda */}
          {favoritesItems.length > 0 && (<div className="relative h-full overflow-hidden rounded-[1.75rem] border border-rose-500/[0.10] bg-gradient-to-br from-rose-950/[0.25] to-transparent p-5 shadow-[inset_0_1px_0_rgba(255,100,100,0.05)] sm:p-6">
              <div className="pointer-events-none absolute -right-16 -top-12 h-48 w-48 rounded-full bg-rose-500/[0.07] blur-[70px]"/>
              <div className="relative flex h-full flex-col gap-3">
                <RailHeader label={uiMessage("ui.33b332595fe4")} subtitle={uiMessage("ui.70004a57858b")} accent="rose" icon={<Heart className="h-3.5 w-3.5 fill-rose-400 text-rose-400"/>} onViewAll={() => scrollToGrid("favorites", "popularity-desc")}/>
                <ScrollRail>
                  {favoritesItems.map((item, i) => (<div key={item.id} className="w-[118px] shrink-0 sm:w-[148px]">
                      <LibraryPosterCard item={item} priority={i < 5} inCustomList={Boolean(listMembership[`${item.tmdb_id}:${item.media_type}`]?.length)}/>
                    </div>))}
                </ScrollRail>
              </div>
            </div>)}
      ...
      {(spotlightItem != null || watchlistItems.length > 0) && (<section className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:gap-6">

          {/* Spotlight */}
          {spotlightItem != null && (<div className="flex min-w-0 flex-col gap-3">
              <RailLabel label={uiMessage("ui.af1cde8c04b9")}/>
              <SpotlightCard item={spotlightItem}/>
            </div>)}

          {/* Watchlist rail */}
          {watchlistItems.length > 0 && (<div className="flex min-w-0 flex-col gap-3 lg:h-full">
              <RailHeader label={uiMessage("ui.f84a0cea3681")} subtitle={uiMessage("ui.be3f81690ea2")} onViewAll={() => scrollToGrid("watchlist", "popularity-desc")}/>
              <div className="relative flex-1 flex flex-col justify-center rounded-[1.5rem] border border-white/[0.08] bg-white/[0.025] p-3 shadow-[0_18px_56px_rgba(0,0,0,0.30)] sm:rounded-[1.75rem] sm:p-4">
                ...
                <ScrollRail>
                  {watchlistItems.map((item, i) => (<div key={item.id} className="w-[124px] shrink-0 sm:w-[175px]">
                      <LibraryPosterCard item={item} priority={i < 5} inCustomList={Boolean(listMembership[`${item.tmdb_id}:${item.media_type}`]?.length)}/>
                    </div>))}
                </ScrollRail>
              </div>
      ...
      {(recentItems.length > 0 || shortestItems.length > 0) && (<section className="relative grid gap-10 lg:grid-cols-2 lg:gap-8">

          {recentItems.length > 0 && (<div className="flex min-w-0 flex-col gap-3">
              <RailHeader label={uiMessage("ui.75a11d86534d")} subtitle={uiMessage("ui.7eb6b29e49c9")} onViewAll={() => scrollToGrid("all", "recent")}/>
              <ScrollRail>
                {recentItems.map((item, i) => (<div key={item.id} className="w-[118px] shrink-0 sm:w-[140px]">
                    <LibraryPosterCard item={item} priority={i < 4} inCustomList={Boolean(listMembership[`${item.tmdb_id}:${item.media_type}`]?.length)}/>
                  </div>))}
              </ScrollRail>
            </div>)}

          {shortestItems.length > 0 && (<div className="flex min-w-0 flex-col gap-3">
              <RailHeader label={uiMessage("ui.204e7e2f417c")} subtitle={uiMessage("ui.a711f64932dc")} onViewAll={() => scrollToGrid("all", "runtime-asc")}/>
              <ScrollRail>
                {shortestItems.map((item, i) => (<div key={item.id} className="w-[118px] shrink-0 sm:w-[140px]">
                    <LibraryPosterCard item={item} priority={i < 4} inCustomList={Boolean(listMembership[`${item.tmdb_id}:${item.media_type}`]?.length)}/>
                  </div>))}
              </ScrollRail>
            </div>)}
      ...
      <section ref={gridSectionRef} className="relative scroll-mt-6 pb-12">
        <SectionHeader eyebrow={uiMessage("ui.52e96f4067cd")} accent="indigo" title={uiMessage("ui.0544643f71c4")} subtitle={uiMessage("ui.457d5be36359")} size="md" action={<span className="rounded-full border border-white/[0.10] bg-white/[0.05] px-3 py-1 text-[11px] font-bold text-white/50">
              {filteredLibrary.length}{" "}
              {filteredLibrary.length === 1 ? uiMessage("ui.8a1be36de69d") : uiMessage("ui.d3e75bbc154a")}
            </span>} className="mb-6"/>

        {/* Busca interna da Biblioteca */}
        <div className="relative mb-4">
          <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35"/>
          <input type="search" inputMode="search" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={uiMessage("ui.64e75461498f")} aria-label={uiMessage("ui.4a8d4465195a")} className="h-11 w-full rounded-full border border-white/[0.10] bg-white/[0.04] pl-11 pr-11 text-sm text-white placeholder:text-white/35 outline-none transition focus:border-indigo-400/40 focus:bg-white/[0.06] [&::-webkit-search-cancel-button]:appearance-none"/>
          {searchQuery && (<button type="button" onClick={() => setSearchQuery("")} aria-label={uiMessage("ui.7a08afed9250")} className="absolute right-3 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-full text-white/45 transition hover:bg-white/[0.10] hover:text-white">
              <X className="h-4 w-4"/>
            </button>)}
        </div>
        ...
        <div className="mb-6 hidden md:block">
          <FullFilterBar activeTab={activeTab} mediaFilter={mediaFilter} yearFilter={yearFilter} sortBy={sortBy} availableYears={availableYears} total={filteredLibrary.length} hasActiveFilters={hasActiveFilters || hasActiveQuery} onTabChange={(t) => { setActiveTab(t); setPage(1); }} onMediaChange={(m) => { setMediaFilter(m); setPage(1); }} onYearChange={(y) => { setYearFilter(y); setPage(1); }} onSortChange={(s) => { setSortBy(s); setPage(1); }} onClear={clearFilters}/>
        </div>

        {/* Mobile filter row */}
        <div className="mb-5 flex items-center justify-between gap-3 md:hidden">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            {activeTab !== "all" && (<span className="rounded-full bg-indigo-500/20 px-2.5 py-0.5 text-[11px] font-medium text-indigo-200">
                {TAB_OPTIONS.find((t) => t.id === activeTab)?.label}
              </span>)}
            {mediaFilter !== "all" && (<span className="rounded-full bg-white/[0.07] px-2.5 py-0.5 text-[11px] text-white/60">
                {MEDIA_OPTIONS.find((m) => m.id === mediaFilter)?.label}
              </span>)}
            {yearFilter !== null && (<span className="rounded-full bg-white/[0.07] px-2.5 py-0.5 text-[11px] text-white/60">
                {yearFilter}
              </span>)}
            {!hasActiveFilters && (<span className="text-[12px] text-white/35">{filteredLibrary.length}{uiMessage("ui.d3e75bbc154a")}</span>)}
          </div>
          <button type="button" onClick={() => setMobileFiltersOpen(true)} className="flex h-9 shrink-0 items-center gap-2 rounded-full border border-white/[0.12] bg-white/[0.05] px-3 text-[11px] font-bold text-white/75 transition hover:bg-white/[0.10]">
        ...
        {/* Grid */}
        {filteredLibrary.length === 0 ? (<LibraryEmptyState activeTab={activeTab} searchQuery={searchQuery.trim() || undefined}/>) : (<div className="flex flex-col gap-12">
            <LibraryGrid items={visibleLibrary} listMembership={listMembership}/>
            {totalPages > 1 && (<LibraryPagination page={safePage} totalPages={totalPages} total={filteredLibrary.length} onPageChange={setPage}/>)}
          </div>)}
      </section>

      {/* ── 7. Em Breve (fim da página) ── */}
      {comingSoonItems.length > 0 && (<section className="relative flex flex-col gap-4 pb-12">
          <RailHeader label={uiMessage("ui.8a6ec37c6474")} subtitle={uiMessage("ui.9b02721b1343")} accent="amber" icon={<Calendar className="h-3.5 w-3.5 text-amber-400"/>} onViewAll={() => scrollToGrid("coming-soon", "release-asc")}/>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
            {comingSoonItems.map((item, i) => (<ComingSoonCard key={item.id} item={item} priority={i < 3}/>))}
          </div>
        </section>)}

      {/* Mobile filters bottom sheet */}
      {mobileFiltersOpen && (<MobileFiltersSheet activeTab={activeTab} mediaFilter={mediaFilter} yearFilter={yearFilter} sortBy={sortBy} availableYears={availableYears} onApply={(tab, media, year, sort) => {
                setActiveTab(tab);
...
const STAT_CONFIGS: {
    ...
    {
        key: "watchlist",
        label: uiMessage("ui.bf271a8c249e"),
        description: uiMessage("ui.50ae045690c9"),
        tab: "watchlist",
    ...
    {
        key: "favorites",
        label: uiMessage("ui.33b332595fe4"),
        description: uiMessage("ui.b6308ccb6a05"),
        tab: "favorites",
        sort: "popularity-desc",
    ...
    {
        key: "watching",
        label: uiMessage("ui.5ca3739c28b7"),
        description: uiMessage("ui.5d0212663502"),
        tab: "watching",
        sort: "recent",
    ...
    {
        key: "comingSoon",
        label: uiMessage("ui.0f6dfca7d303"),
        description: uiMessage("ui.c2a8d2ebb28e"),
        tab: "coming-soon",
        sort: "release-asc",
    ...
    },
    {
        key: "watched",
        label: "Assistidos",
        description: uiMessage("ui.22eb889fda61"),
        tab: "watched",
        sort: "release-desc",
    ...
    },
    {
        key: "fridge",
        label: "Geladeira",
        description: uiMessage("ui.c99fe7c4a211"),
        tab: "fridge",
        sort: "recent",
    ...
    },
    {
        key: "abandoned",
        label: "Abandonados",
        description: uiMessage("ui.d899b84ef6f9"),
        tab: "abandoned",
        sort: "recent",
        icon: <X className="h-4 w-4 text-rose-300"/>,
    },
];
function LibraryStatsRow({ stats, onStatClick, }: {
    stats: ExtendedStats;
    ...
    return (<div className="relative">
      <div ref={scrollerRef} className="-mx-4 overflow-x-auto no-scrollbar px-4 pb-1 scroll-smooth sm:mx-0 sm:overflow-x-visible sm:px-0 sm:pb-0">
        <div className="flex w-max gap-2 sm:grid sm:w-auto sm:grid-cols-7 sm:gap-3">
        {STAT_CONFIGS.map((config) => (<button key={config.key} type="button" onClick={() => onStatClick(config.tab, config.sort)} className="group flex w-[92px] shrink-0 touch-pan-x flex-col gap-2 rounded-[1.1rem] border border-white/[0.08] bg-white/[0.04] px-2.5 py-3 text-left transition hover:border-white/[0.14] hover:bg-white/[0.07] min-[390px]:w-[94px] sm:w-auto sm:rounded-[1.25rem] sm:px-4 sm:py-3.5">
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center sm:hidden">
        <button type="button" aria-label={uiMessage("ui.6409e1339e58")} disabled={!canScrollLeft} onClick={() => scrollStats(-1)} className="pointer-events-auto flex h-8 w-8 items-center justify-center rounded-full border border-white/[0.12] bg-[#09090f]/85 text-white/70 shadow-[0_10px_28px_rgba(0,0,0,0.35)] backdrop-blur-md transition disabled:opacity-0">
          <ChevronLeft className="h-4 w-4"/>
        </button>
      </div>

      <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center sm:hidden">
        <button type="button" aria-label={uiMessage("ui.a52a2029e55b")} disabled={!canScrollRight} onClick={() => scrollStats(1)} className="pointer-events-auto flex h-8 w-8 items-center justify-center rounded-full border border-white/[0.12] bg-[#09090f]/85 text-white/70 shadow-[0_10px_28px_rgba(0,0,0,0.35)] backdrop-blur-md transition disabled:opacity-0">
...
function RailLabel({ label }: {
    ...
    return (<p className="text-[10px] font-black uppercase tracking-[0.22em] text-indigo-200/65 sm:text-[11px]">
      {label}
    </p>);
}
function RailHeader({ label, subtitle, accent = "indigo", icon, onViewAll, }: {
    label: string;
    subtitle?: string;
    ...
    return (<div className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        {subtitle && (<p className="mt-0.5 line-clamp-1 text-[11px] text-white/38">{subtitle}</p>)}
      </div>
      {onViewAll && (<button type="button" onClick={onViewAll} className={`flex h-7 shrink-0 items-center gap-1 rounded-full px-1.5 text-[11px] font-bold sm:text-[12px] ${accentText} hover:underline`}>{uiMessage("ui.7a394d39e682")}<ChevronRight className="h-3.5 w-3.5"/>
        </button>)}
    </div>);
}
// ── Scroll Rail ───────────────────────────────────────────────────────────────
function ScrollRail({ children, className, }: {
    ...
    return (<div className="relative">
      {canLeft && (<button type="button" onClick={() => scroll(-1)} aria-label="Anterior" className="absolute -left-3 top-1/2 z-10 hidden h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full border border-white/[0.14] bg-black/75 text-white/80 shadow-lg backdrop-blur-sm transition hover:bg-black/95 hover:text-white sm:flex">
          <ChevronLeft className="h-4 w-4"/>
        </button>)}
      <div ref={ref} className={`flex gap-3 overflow-x-auto no-scrollbar pb-3 ${className ?? ""}`}>
        {children}
      </div>
      {canRight && (<button type="button" onClick={() => scroll(1)} aria-label={uiMessage("ui.64b2d0d63475")} className="absolute -right-3 top-1/2 z-10 hidden h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full border border-white/[0.14] bg-black/75 text-white/80 shadow-lg backdrop-blur-sm transition hover:bg-black/95 hover:text-white sm:flex">
          <ChevronRight className="h-4 w-4"/>
        </button>)}
    </div>);
}
// ── Coming Soon Card ──────────────────────────────────────────────────────────
function ComingSoonCard({ item, priority, }: {
    ...
    const type = item.media_type === "movie" ? "Filme" : uiMessage("ui.74f19285073a");
    ...
    return (<Link href={titleHref} className="group block">
      <div className="relative overflow-hidden rounded-[1.35rem] border border-white/[0.07] bg-[#07080f] shadow-[0_12px_40px_rgba(0,0,0,0.52)] transition duration-300 group-hover:border-amber-300/[0.18] group-hover:shadow-[0_20px_60px_rgba(0,0,0,0.68)]" style={{ aspectRatio: "16/9" }}>
        ...
        {typeof rating === "number" && rating > 0 && (<div className="absolute right-3 top-3 rounded-full border border-amber-200/[0.18] bg-black/60 px-2 py-0.5 text-[10px] font-black text-amber-100 backdrop-blur-sm">
            ★ {rating.toFixed(1)}
          </div>)}

        <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4">
          {releaseLabel && (<p className="mb-1 text-[9px] font-black uppercase tracking-[0.20em] text-amber-300/75">
              {releaseLabel}
            </p>)}
          <LocalizedTitle as="h3" variant="compact" title={displayTitle} originalTitle={title?.original_title} className="sm:[&>span:first-child]:text-[15px]"/>
...
function SpotlightCard({ item }: {
    ...
    const spotlightProviderName = getCanonicalProviderDisplayName({ name: item.best_provider_name }) ?? item.best_provider_name;
    ...
    const metaParts: string[] = [
        title?.year?.toString() ?? "",
        item.media_type === "movie" ? "Filme" : uiMessage("ui.74f19285073a"),
    ...
    return (<Link href={titleHref} className="group block h-full">
      <div className="relative min-h-[300px] overflow-hidden rounded-[1.5rem] border border-white/[0.08] bg-black/40 shadow-[0_24px_80px_rgba(0,0,0,0.55)] transition duration-300 group-hover:border-white/[0.16] sm:min-h-[380px] sm:rounded-[1.75rem]">
        ...
        <div className="absolute left-3 top-3 sm:left-4 sm:top-4">
          <span className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.16em] shadow-[0_6px_18px_rgba(0,0,0,0.40)] backdrop-blur-md ${badgeClass}`}>
            {badgeLabel}
          </span>
        </div>
        ...
        {(spotlightProviderLogo || spotlightProviderName) && (<div className="absolute right-3 top-3 overflow-hidden rounded-lg border border-white/[0.14] bg-black/55 shadow-[0_4px_14px_rgba(0,0,0,0.45)] backdrop-blur-md sm:right-4 sm:top-4">
            {spotlightProviderLogo ? (<Image src={spotlightProviderLogo} alt={spotlightProviderName ?? ""} width={28} height={28} unoptimized className="h-7 w-7 object-cover"/>) : (<span className="block max-w-[110px] truncate px-2 py-1 text-[9px] font-black uppercase tracking-[0.08em] text-white/85">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6">
          {metaParts.length > 0 && (<p className="mb-2 text-[11px] font-medium text-white/45">
              {metaParts.join(" · ")}
            </p>)}
          <LocalizedTitle as="h2" variant="large" title={displayTitle} originalTitle={title?.original_title} className="[&>span:first-child]:text-[22px] sm:[&>span:first-child]:text-3xl"/>

          {isWatching && progress !== null && progress > 0 && (<div className="mt-4">
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <span className="text-[11px] text-white/40">
                  {Math.round(progress)}{uiMessage("ui.5795f0c201ac")}</span>
              </div>
              <div className="h-[3px] overflow-hidden rounded-full bg-white/[0.12]">
                <div className="h-full bg-gradient-to-r from-violet-500 to-indigo-400 shadow-[0_0_8px_rgba(139,92,246,0.60)]" style={{ width: `${Math.min(100, progress)}%` }}/>
              </div>
            </div>)}
        </div>
      </div>
...
function FullFilterBar({ activeTab, mediaFilter, yearFilter, sortBy, availableYears, total, hasActiveFilters, onTabChange, onMediaChange, onYearChange, onSortChange, onClear, }: {
    ...
    return (<div className="flex flex-wrap items-center gap-2 rounded-[1.25rem] border border-white/[0.08] bg-white/[0.03] px-4 py-3">

      {/* Estado */}
      <FilterSelect label="Estado" value={activeTab} options={TAB_OPTIONS.map((t) => ({ value: t.id, label: t.label }))} onChange={(v) => onTabChange(v as LibraryTab)}/>

      <FilterDivider />

      {/* Tipo */}
      <FilterSelect label="Tipo" value={mediaFilter} options={MEDIA_OPTIONS.map((m) => ({ value: m.id, label: m.label }))} onChange={(v) => onMediaChange(v as MediaFilter)}/>

      {availableYears.length > 1 && (<>
          <FilterDivider />
          {/* Ano */}
          <FilterSelect label="Ano" value={yearFilter?.toString() ?? "all"} options={[
      ...
      {/* Ordenar por */}
      <FilterSelect label={uiMessage("ui.1f77dbb2bd3d")} value={sortBy} options={SORT_OPTIONS.map((s) => ({ value: s.value, label: s.label }))} onChange={(v) => onSortChange(v as SortBy)} highlight/>

      {/* Clear filters */}
      {hasActiveFilters && (<button type="button" onClick={onClear} className="ml-1 rounded-full border border-white/[0.08] px-3 py-1 text-[11px] font-medium text-white/40 transition hover:border-white/[0.18] hover:text-white/70">{uiMessage("ui.d22f68306a54")}</button>)}

      {/* Count */}
      <p className="ml-auto text-[12px] text-white/30">
        {total} {total === 1 ? uiMessage("ui.8a1be36de69d") : uiMessage("ui.d3e75bbc154a")}
      </p>
    </div>);
}
...
function FilterSelect({ label, value, options, onChange, highlight = false, }: {
    label: string;
    value: string;
    options: {
        value: string;
    ...
    return (<div className="relative flex items-center gap-1.5">
      ...
      <div className="relative">
        <select value={value} onChange={(e) => onChange(e.target.value)} style={{ colorScheme: "dark" }} className={[
        ...
        ].filter(Boolean).join(" ")} aria-label={label}>
...
// ── Mobile filters bottom sheet ───────────────────────────────────────────────
function MobileFiltersSheet({ activeTab, mediaFilter, yearFilter, sortBy, availableYears, onApply, onClose, }: {
    activeTab: LibraryTab;
    mediaFilter: MediaFilter;
    yearFilter: number | null;
    sortBy: SortBy;
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 max-h-[88vh] overflow-y-auto rounded-t-[1.75rem] border-t border-white/[0.08] bg-[#0d0d16] pb-safe-area-inset-bottom">
        ...
        <div className="px-5 pb-6">
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-base font-black text-white">{uiMessage("ui.245fda308246")}</h3>
            <button type="button" onClick={onClose} className="rounded-full border border-white/[0.10] p-1.5 text-white/50">
              <X className="h-4 w-4"/>
            </button>
          </div>
          ...
          <SheetSection label="Estado">
            <div className="flex flex-wrap gap-2">
              {TAB_OPTIONS.map((tab) => (<button key={tab.id} type="button" onClick={() => setLocalTab(tab.id)} className={[
          ...
          <SheetSection label="Tipo">
            <div className="flex gap-2">
              {MEDIA_OPTIONS.map((opt) => (<button key={opt.id} type="button" onClick={() => setLocalMedia(opt.id)} className={[
                "rounded-full border px-3.5 py-1.5 text-[12px] font-medium transition",
                localMedia === opt.id
                    ? "border-indigo-400/40 bg-indigo-500/20 text-indigo-100"
                    : "border-white/[0.10] text-white/55 hover:bg-white/[0.07]",
          ...
          {/* Ano */}
          {availableYears.length > 1 && (<SheetSection label="Ano">
              <select value={localYear?.toString() ?? "all"} onChange={(e) => setLocalYear(e.target.value === "all" ? null : parseInt(e.target.value, 10))} style={{ colorScheme: "dark" }} className="h-10 w-full appearance-none rounded-2xl border border-white/[0.08] bg-white/[0.04] px-4 text-sm text-white outline-none [&_option]:bg-[#0d0d14]">
                <option value="all">{uiMessage("ui.80fa8ab35d89")}</option>
                {availableYears.map((y) => (<option key={y} value={y}>{y}</option>))}
              </select>
            </SheetSection>)}

          {/* Ordenar por */}
          <SheetSection label={uiMessage("ui.1f77dbb2bd3d")}>
            <div className="flex flex-wrap gap-2">
              {SORT_OPTIONS.map((opt) => (<button key={opt.value} type="button" onClick={() => setLocalSort(opt.value)} className={[
                "rounded-full border px-3.5 py-1.5 text-[12px] font-medium transition",
                localSort === opt.value
                    ? "border-indigo-400/40 bg-indigo-500/20 text-indigo-100"
                    : "border-white/[0.10] text-white/55 hover:bg-white/[0.07]",
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button type="button" onClick={() => onApply(localTab, localMedia, localYear, localSort)} className="flex-1 rounded-2xl bg-indigo-500 py-3 text-sm font-black text-white transition hover:bg-indigo-400">
              Aplicar
            </button>
          </div>
        </div>
...
// ── Pagination ────────────────────────────────────────────────────────────────
function LibraryPagination({ page, totalPages, total, onPageChange, }: {
    page: number;
    totalPages: number;
    total: number;
    ...
    if (totalPages <= 1) {
        return (<p className="text-center text-[11px] font-medium text-white/28">
        {total} {total === 1 ? uiMessage("ui.8a1be36de69d") : uiMessage("ui.d3e75bbc154a")}
      </p>);
    }
    const pages = getPaginationPages(page, totalPages);
    return (<div className="flex flex-wrap items-center justify-center gap-1.5">
      <p className="mr-1 hidden text-[11px] font-medium text-white/28 sm:block">
        {total}{uiMessage("ui.d3e75bbc154a")}</p>

      <button type="button" disabled={page === 1} onClick={() => onPageChange(page - 1)} className="h-9 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 text-xs font-black text-white/45 transition hover:border-white/[0.15] hover:text-white disabled:cursor-not-allowed disabled:opacity-25">
        ‹
      </button>

      {pages.map((item, index) => item === "gap" ? (<span key={`gap-${index}`} className="px-1 text-xs text-white/22">…</span>) : (<button key={item} type="button" onClick={() => onPageChange(item)} className={[
      ...
      <button type="button" disabled={page === totalPages} onClick={() => onPageChange(page + 1)} className="h-9 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 text-xs font-black text-white/45 transition hover:border-white/[0.15] hover:text-white disabled:cursor-not-allowed disabled:opacity-25">
        ›
      </button>
    </div>);
}
...
function useWatchlistHydration(library: Poplog3UserLibraryItem[], activeTab: string) {
    ...
    useEffect(() => {
        if (activeTab !== "watchlist")
            return;
        const hasTvWatchlist = library.some((item) => item.media_type === "tv" && isPureWatchlist(item));
        if (!hasTvWatchlist)
            return;
...
// ── Sorting ───────────────────────────────────────────────────────────────────
function sortLibraryItems(a: Poplog3UserLibraryItem, b: Poplog3UserLibraryItem, sortBy: SortBy, randomSeed: number | null, traktScoreMap: TraktScoreMap) {
    switch (sortBy) {
...
function compareRandom(a: Poplog3UserLibraryItem, b: Poplog3UserLibraryItem, seed: number | null) {
    if (seed === null)
        return 0;
...
function compareReleaseTime(a: Poplog3UserLibraryItem, b: Poplog3UserLibraryItem, direction: "asc" | "desc") {
...
function isMarathoning(item: Poplog3UserLibraryItem): boolean {
    ...
    if (item.media_type !== "tv")
        return false;
    if (item.status === "abandoned" || item.status === "fridge")
        return false;
    const pending = getPendingEpisodeCount(item);
    const hasStarted = item.status === "watching" ||
        item.status === "watched" ||
...
function isCompletedOrUpToDate(item: Poplog3UserLibraryItem): boolean {
    ...
    if (item.status === "abandoned" || item.status === "fridge")
        return false;
    const pending = getPendingEpisodeCount(item);
    const hasStartedOrCompleted = item.status === "watching" ||
        item.status === "watched" ||
        (item.watched_episodes ?? 0) > 0 ||
...
function getPendingEpisodeCount(item: Poplog3UserLibraryItem): number | null {
    ...
    if (typeof watched === "number" && Number.isFinite(watched) && watched >= 0 &&
