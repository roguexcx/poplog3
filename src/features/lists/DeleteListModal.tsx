...
export default function DeleteListModal({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[120] flex items-end justify-center bg-black/74 px-3 py-3 backdrop-blur-sm sm:items-center sm:px-5 sm:py-6"
      role="alertdialog"
      aria-modal="true"
    ...
    >
      <div
        className="relative w-full max-w-md overflow-hidden rounded-[1.75rem] border border-rose-300/[0.16] bg-[#0b090d] p-6 shadow-[0_28px_100px_rgba(0,0,0,0.72)]"
      ...
      >
        <button
          ...
          onClick={onClose}
          disabled={deleting}
          className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full border border-white/[0.10] bg-white/[0.04] text-white/50 transition hover:bg-white/[0.08] hover:text-white disabled:opacity-40"
          aria-label="Fechar"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <h2 id={titleId} className="mt-5 pr-10 text-xl font-black tracking-[-0.035em] text-white">Excluir “{listName}”?</h2>
        <p className="mt-3 text-sm leading-6 text-white/45">
          Os itens sairão desta coleção. Watchlist, favoritos, progresso e títulos assistidos continuarão intactos.
        </p>
