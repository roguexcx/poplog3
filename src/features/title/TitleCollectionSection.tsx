...
function HorizontalCard({ part, index, isCurrent, }: {
    ...
    const inner = (<article className={[
      ...
      {part.posterPath && (<div className="absolute inset-0">
          <TmdbImage path={part.posterPath} fallbackPath={null} size="w500" alt="" fallbackLabel="" className="h-full w-full object-cover scale-[1.12] blur-[8px] brightness-[0.28] saturate-[1.3]"/>
        </div>)}
      ...
      <div className="relative flex h-full items-center gap-4 px-4 py-3">
        ...
        <div className="relative ml-4 h-[calc(100%-12px)] shrink-0 overflow-hidden rounded-[0.65rem] shadow-[0_8px_24px_rgba(0,0,0,0.6)]" style={{ aspectRatio: "2/3" }}>
          <TmdbImage path={part.posterPath ?? null} fallbackPath={null} size="w500" alt={part.title} fallbackLabel={part.title} className="h-full w-full object-cover"/>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          {part.year && (<p className="mt-1 text-[11px] text-white/38">{part.year}</p>)}
          {isCurrent && (<span className="mt-2 inline-flex items-center rounded-full border border-indigo-400/45 bg-indigo-500/20 px-2 py-[3px] text-[8px] font-black uppercase tracking-[0.18em] text-indigo-200 whitespace-nowrap">{uiMessage("ui.39e48577bf66")}</span>)}
        </div>

        {/* action buttons on hover */}
        <div className="flex shrink-0 flex-col gap-1.5 opacity-0 transition duration-200 group-hover:opacity-100" onClick={(e) => e.stopPropagation()}>
          <CardActionButton onClick={() => handleAction(inWatchlist ? "removeFromWatchlist" : "addToWatchlist")} disabled={!isLoggedIn || saving} title={inWatchlist ? "Remover da watchlist" : "Adicionar à watchlist"} active={inWatchlist} saving={saving} activeClass="border-sky-400/55 bg-sky-400/[0.18] text-sky-300">
            <IconBookmark filled={inWatchlist}/>
          </CardActionButton>
          <CardActionButton onClick={() => handleAction(isWatched ? "markAsUnwatched" : "markAsWatched")} disabled={!isLoggedIn || saving} title={isWatched ? "Desmarcar" : uiMessage("ui.56c6eef6ab5b")} active={isWatched} saving={saving} activeClass="border-emerald-400/55 bg-emerald-400/[0.18] text-emerald-300">
            <IconCheck />
          </CardActionButton>
          <CardActionButton onClick={feedback.toggleNotInterested} disabled={!isLoggedIn || feedback.saving} title={feedback.notInterested ? "Remover sem interesse" : uiMessage("ui.7aea725c0eb9")} active={feedback.notInterested} saving={feedback.saving} activeClass="border-rose-400/55 bg-rose-400/[0.18] text-rose-300">
            <IconX />
          </CardActionButton>
        </div>
      </div>
...
// ─── Poster card (≥ 4 films) ──────────────────────────────────────────────────
function PosterCard({ part, index, isCurrent, }: {
    part: TitleCollectionPart;
    index: number;
    isCurrent: boolean;
}) {
    const { isLoggedIn, inWatchlist, isWatched, saving, handleAction, feedback } = useCollectionPartActions(part);
    const href = isCurrent ? undefined : `/title/${part.mediaType ?? "movie"}/${part.id}`;
    const card = (<article className={`group relative ${isCurrent ? "ring-2 ring-indigo-400/50 ring-offset-[3px] ring-offset-[#020617] rounded-[1.2rem]" : ""}`}>
      {/* sequence number */}
      <div className={[
      ...
      <div className="relative overflow-hidden rounded-[1.2rem] border border-white/[0.08] bg-white/[0.035] shadow-[0_14px_40px_rgba(0,0,0,0.32)] transition duration-300 group-hover:-translate-y-1 group-hover:border-indigo-400/30 group-hover:shadow-[0_20px_56px_rgba(0,0,0,0.48)]">
        <div className="relative aspect-[2/3] overflow-hidden bg-white/[0.04]">
          <TmdbImage path={part.posterPath ?? null} fallbackPath={null} size="w500" alt={part.title} fallbackLabel={part.title} className="h-full w-full object-cover brightness-[0.90] saturate-[1.04] transition duration-500 group-hover:scale-[1.045] group-hover:brightness-100"/>
          ...
          {/* action buttons */}
          <div className="absolute inset-x-0 bottom-0 p-2.5 opacity-0 transition duration-300 group-hover:opacity-100" onClick={(e) => e.stopPropagation()}>
            <div className="flex gap-1.5">
              ...
              <CardActionButton onClick={feedback.toggleNotInterested} disabled={!isLoggedIn || feedback.saving} title={feedback.notInterested ? "Remover" : uiMessage("ui.7aea725c0eb9")} active={feedback.notInterested} saving={feedback.saving} activeClass="border-rose-400/55 bg-rose-400/[0.18] text-rose-300">
                <IconX />
              </CardActionButton>
            </div>
          </div>
      ...
      <div className="px-0.5 pt-2.5">
        ...
        {part.year && (<p className="mt-1 text-[11px] text-white/32">{part.year}</p>)}
        {isCurrent && (<span className="mt-1.5 inline-flex items-center rounded-full border border-indigo-400/45 bg-indigo-500/[0.15] px-2 py-[3px] text-[8px] font-black uppercase tracking-[0.18em] text-indigo-300 whitespace-nowrap">{uiMessage("ui.39e48577bf66")}</span>)}
      </div>
    </article>);
    return href ? (<Link href={href} className="block">{card}</Link>) : (<div>{card}</div>);
}
// ─── Section ──────────────────────────────────────────────────────────────────
export default function TitleCollectionSection({ collection, currentTitleId, }: TitleCollectionSectionProps) {
    ...
    const hasChronologicalOrder = Boolean(collection?.chronologicalOrder?.length) &&
        collection?.chronologicalOrder?.length === rawParts.length;
    const parts = useMemo(() => {
        const order = orderMode === "chronological" && hasChronologicalOrder
    ...
    return (<div className="relative overflow-hidden rounded-[1.75rem] border border-indigo-500/[0.18] bg-[linear-gradient(135deg,rgba(99,102,241,0.09)_0%,rgba(139,92,246,0.05)_50%,rgba(17,24,39,0.30)_100%)] p-5 shadow-[inset_0_1px_0_rgba(165,180,252,0.08)] backdrop-blur-sm sm:p-6">
      {/* ambient glow */}
      ...
      <div className="relative mb-5 flex items-center gap-3 sm:mb-6">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {hasChronologicalOrder && (<div className="hidden rounded-full border border-indigo-400/20 bg-black/30 p-0.5 sm:flex">
              {[
                ["release", uiMessage("ui.04c60fd2a859")],
                ["chronological", uiMessage("ui.0c10e65b6432")],
            ].map(([mode, label]) => (<button key={mode} type="button" onClick={() => setOrderMode(mode as "release" | "chronological")} className={[
          ...
          <span className="rounded-full border border-indigo-400/20 bg-indigo-500/[0.10] px-2.5 py-1 text-[10px] font-black text-indigo-200/70">
            {count} {count === 1 ? (allMovies ? "filme" : uiMessage("ui.8a1be36de69d")) : (allMovies ? "filmes" : uiMessage("ui.d3e75bbc154a"))}
          </span>
        </div>
      </div>
      ...
      <div className={`relative grid gap-3 ${gridCols}`}>
        {parts.map((part, index) => useHorizontal ? (<HorizontalCard key={part.id} part={part} index={index} isCurrent={part.id === currentTitleId}/>) : (<PosterCard key={part.id} part={part} index={index} isCurrent={part.id === currentTitleId}/>))}
      </div>
    </div>);
}
