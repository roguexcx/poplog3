...
function SectionLabel({ children }: { children: string }) {
  return (
    <div className="px-4 pb-1 pt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-white/28">
      {children}
    </div>
  );
}
...
export default function SearchBar({
  ...
  useEffect(() => {
    setActiveIndex((current) => {
      if (actionOptions.length === 0) return 0;
    ...
    >
      <div className="relative">
        ...
        <input
          ...
          aria-activedescendant={
            activeOption ? optionDomId(listboxId, activeOption.key) : undefined
          }
          aria-autocomplete="list"
          aria-label="Buscar filmes, séries, pessoas e conteúdos"
          placeholder={placeholder ?? "Buscar filmes, séries e pessoas..."}
          value={query}
      ...
      {showDropdown && (
        <div id={listboxId} role="listbox" className={dropdownClassName}>
          {loading && (
            <p className="px-4 py-3 text-[13px] text-white/35">Buscando...</p>
          )}
          ...
          {!loading && titleOptions.length > 0 && (
            <>
              ...
              {titleOptions.map((option) => {
                ...
                return (
                  <Link
                    id={optionDomId(listboxId, option.key)}
                    key={option.key}
                    href={option.href}
                    role="option"
                    ...
                    onMouseEnter={() => activateOption(option.key)}
                    onClick={closeSearch}
                  >
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                      ...
                      {rating && (
                        <p className="mt-2 text-[11px] font-semibold text-yellow-400/70">
                          {rating}
                        </p>
                      )}
                    </div>
                  ...
                  >
                    <div className="relative flex h-[58px] w-[58px] shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white/[0.05]">
                      <TmdbImage
                        ...
                        sizes="58px"
                        className="object-cover opacity-90"
                        fallback={<UserRound className="size-5 text-white/25" aria-hidden />}
                      />
                    </div>
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                      ...
                      {knownFor && (
                        <p className="mt-1.5 line-clamp-1 text-[11px] text-white/30">
                          {knownFor}
                        </p>
                      )}
                    </div>
                  ...
                  >
                    <div className="relative flex h-[52px] w-[52px] shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white/[0.05]">
                      <TmdbImage
                        ...
                        fallback={<Building2 className="size-5 text-white/25" aria-hidden />}
                      />
                    </div>
          ...
          {!loading && actionOptions.length === 0 && (
            <div className="px-4 py-4 text-[13px] text-white/35">
              Nenhum resultado rápido.
            </div>
          )}

          <Link
            href={searchHref(trimmedQuery)}
            className="flex items-center justify-center gap-1.5 border-t border-white/[0.05] px-4 py-2.5 text-[11px] text-white/40 transition hover:bg-white/[0.04] hover:text-white/60"
            onClick={closeSearch}
          >
