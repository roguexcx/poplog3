...
const CHIP_BASE =
  "flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11.5px] font-semibold transition sm:shrink sm:px-3.5 sm:py-2 sm:text-[12px]";
...
function Chip({ item, active, onSelect }: { item: ChipItem; active: boolean; onSelect: (id: string) => void }) {
  return (
    <button
      type="button"
      onClick={() => onSelect(item.id)}
      className={[CHIP_BASE, active ? ACCENT_ACTIVE[item.accent] : CHIP_INACTIVE].join(" ")}
    >
...
function ChipRow({ chips, activeSlug, onSelect }: { chips: ChipItem[]; activeSlug: string; onSelect: (id: string) => void }) {
  return (
    <div className="-mx-4 overflow-x-auto px-4 sm:mx-0 sm:overflow-visible sm:px-0">
      <div className="flex gap-2 sm:flex-wrap">
        {chips.map((chip) => (
          <Chip key={chip.id} item={chip} active={activeSlug === chip.id} onSelect={onSelect} />
        ))}
      </div>
    </div>
  );
