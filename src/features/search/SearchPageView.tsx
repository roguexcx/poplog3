...
function PeopleSection({ people }: { people: SearchPerson[] }) {
...
function CompaniesSection({ companies }: { companies: SearchCompany[] }) {
  ...
  return (
    <section className="space-y-5">
      <SectionHeader
        ...
        subtitle="Estúdios e produtoras relacionados à busca."
        accent="indigo"
      />
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {companies.map((company) => (
          <div
            ...
            className="flex gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3"
          >
            <div className="relative flex size-[52px] shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white/[0.06]">
              {company.logo_path ? (
                <Image
                  src={resolveCatalogImage(company.logo_path, "w92") ?? ""}
                  alt={company.name}
                  fill
                  sizes="52px"
                ...
                <Building2 className="size-5 text-white/35" />
              )}
            </div>
            <div className="min-w-0 py-1">
              <p className="truncate text-[13px] font-bold text-white/90">{company.name}</p>
              {company.origin_country ? (
                <p className="mt-0.5 text-[11px] text-white/40">{company.origin_country}</p>
              ) : null}
              {company.description ? (
                <p className="mt-1 line-clamp-2 text-[11px] leading-snug text-white/35">{company.description}</p>
              ) : null}
            </div>
          </div>
        ))}
...
export default function SearchPageView({
  ...
  const searchSubtitle = useMemo(() => {
    ...
    const parts: string[] = [];
    if (titlesCount > 0) parts.push(`${titlesCount} título${titlesCount !== 1 ? "s" : ""}`);
    if (peopleCount > 0) parts.push(`${peopleCount} pessoa${peopleCount !== 1 ? "s" : ""}`);
    if (parts.length === 0) return "Nenhum resultado";
    return parts.join(" · ");
    ...
    async function load() {
      try {
        ...
        const res = await fetch(
          `/api/poplog3/discovery/shortcut/${encodeURIComponent(selectedShortcut)}?${params}`,
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          setCompanies([]);
          setSearchMeta(null);
          setSearchError("Não foi possível carregar a busca agora. Tente novamente em instantes.");
          setHasSearched(true);
          return;
        }
  ...
  return (
    <PageShell variant="wide">
      <div className="flex flex-col gap-6 py-4 pb-2 sm:gap-8 sm:py-6 sm:pb-8">
        ...
        <section className="sticky top-3 z-40 overflow-hidden rounded-[1.15rem] border border-white/[0.09] bg-[#080b1d]/88 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.34)] backdrop-blur-2xl sm:top-4 sm:rounded-[1.75rem] sm:p-6 md:p-8 lg:rounded-[2rem] lg:p-10">
          ...
          <div className="relative grid gap-4 sm:gap-6 xl:grid-cols-[1.1fr_0.9fr] xl:items-center">
            <div className="hidden sm:order-1 sm:block">
              ...
              <h1 className="text-xl font-black tracking-[-0.04em] text-white sm:text-4xl md:text-5xl lg:text-[3.25rem]">
              ...
              <p className="mt-1.5 max-w-xl text-xs leading-5 text-white/45 sm:mt-3 sm:text-sm md:mt-4 md:text-base md:leading-7">
                Títulos, pessoas, estúdios e tendências da semana.
              </p>
            </div>
            <div className="order-1 flex flex-col gap-2.5 sm:order-2 sm:gap-3">
              <div className="flex items-center gap-2 sm:hidden">
                ...
                <span className="text-[9px] font-bold uppercase tracking-[0.22em] text-indigo-200/80">
                  Buscar &amp; Explorar
                </span>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Título, pessoa, estúdio ou saga..."
                ...
                {(
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center gap-1.5 rounded-full border px-3 py-2 text-[10px] font-black uppercase tracking-[0.10em] transition sm:justify-start sm:px-4 sm:text-[11px] sm:tracking-[0.12em] ${
                        active
        ...
        {!searchError && !loading && hasSearched ? (
          <div className="flex flex-col gap-10">
                  ...
                  {searchMeta && searchMeta.totalPages > 1 ? (
                    <div className="flex flex-col gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3 sm:flex-row sm:items-center sm:justify-between">
                      <p className="text-xs font-semibold text-white/45">
                        Página {searchMeta.page} de {searchMeta.totalPages}
                      </p>
                      <div className="grid grid-cols-2 gap-2 sm:flex">
                        ...
                        <button
                          type="button"
                          onClick={() => setPage((v) => Math.min(maxSearchPage, v + 1))}
            ...
            {!hasAnyResult ? (
              <EmptyState
                ...
                description="Tente outro termo, tipo ou vibe."
                accent="neutral"
              />
            ) : null}
          </div>
              ...
              return (
                <div className="flex flex-col gap-10">
                  {d
                    ? (() => {
                        console.log(
                          `[search/shortcut] slug=${selectedShortcut} type=${type} source=${shortcutData.source} before=${d.beforeFilter} after=${d.afterFilter} fallback=${shortcutData.fallbackUsed} fallbackItems=${d.fallbackItems} slugs=${d.slugs?.join(",")}`
                        );
                        return null;
                      })()
                    : null}
                  {shortcutData.fallbackUsed ? (
                    <div className="rounded-xl border border-amber-400/20 bg-amber-400/[0.06] px-4 py-2.5 text-[11px] font-medium text-amber-200/70">
                      Resultado ampliado com títulos populares do gênero — índice retornou poucos itens para &quot;
                      {shortcutData.title}&quot; nesta janela.
                    </div>
                  ) : null}
                  {hasMovies ? (
                    <section className="space-y-5">
                  ...
                  {!hasMovies && !hasSeries ? (
                    <EmptyState
                      kicker={sc?.label ?? selectedShortcut}
                      title="Sem títulos disponíveis."
                      description="Nenhum resultado encontrado para essa categoria no momento."
                      accent="neutral"
                    />
                  ) : null}
                </div>
                  ...
                  ? (() => {
                      ...
                      return merged.length ? (
                        <section className="space-y-5">
                          <SectionHeader
                            eyebrow="Em destaque"
                            title="Filmes em alta e populares"
                            subtitle="Filmes ganhando atenção e os mais assistidos agora."
                            accent="indigo"
                          />
                    ...
                    {discovery.trending?.filter((t) => t.media_type === "tv").length ? (
                      <section className="space-y-5">
                        <SectionHeader
                          ...
                          subtitle="Séries ganhando atenção agora."
                          accent="indigo"
                        />
                    ...
                    {discovery.popularSeries?.length ? (
                      <section className="space-y-5">
                        <SectionHeader
                          ...
                          subtitle="As séries mais assistidas agora."
                          accent="indigo"
                        />
                    ...
                    {discovery.trending?.length ? (
                      <section className="space-y-5">
                        <SectionHeader
                          eyebrow="Tendências"
                          title="Em alta hoje"
                          subtitle="Filmes e séries ganhando atenção agora."
                          accent="indigo"
                        />
