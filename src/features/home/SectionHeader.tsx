...
function ForYouActions({ item }: { item: ForYouItem }) {
  ...
  return (
    <div className="absolute right-2.5 top-2.5 z-40 flex gap-1.5">
      <button
        type="button"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); watchlist.toggle(); }}
      ...
      >
        {watchlist.saving ? <span className="text-[10px]">…</span> : <IconBookmark filled={watchlist.inWatchlist} />}
      </button>

      <button
        type="button"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); watched.toggle(); }}
...
function FeaturedForYouCard({ item }: { item: ForYouItem }) {
  ...
  return (
    <article className="group relative h-[320px] overflow-hidden rounded-[1.65rem] border border-white/10 bg-white/[0.04] shadow-[0_20px_80px_rgba(0,0,0,0.42)] transition duration-300 hover:-translate-y-1 hover:border-sky-300/40 hover:shadow-[0_24px_90px_rgba(56,189,248,0.16)]">
      <Link href={`/title/${item.media_type}/${item.id}`} className="absolute inset-0 z-10" />
      <ForYouActions item={item} />

      {backdropUrl && (
        <Image src={backdropUrl} alt={item.title_label} fill sizes="720px" className="object-cover object-center transition duration-700 group-hover:scale-105" priority />
      )}

      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-[#020617] from-0% via-[#020617]/98 via-48% to-transparent to-86%" />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#020617] from-0% via-black/35 via-42% to-transparent" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_74%_45%,transparent_0%,rgba(2,6,23,0.18)_46%,rgba(2,6,23,0.72)_100%)]" />
      <div className="pointer-events-none absolute inset-0 opacity-0 shadow-[inset_0_0_0_1px_rgba(125,211,252,0.3)] transition duration-300 group-hover:opacity-100" />

      <div className={["pointer-events-none absolute inset-y-0 left-0 z-20 flex flex-col justify-center p-7", isLongTitle || isLongOverview ? "w-[66%]" : "w-[58%]"].join(" ")}>
        <div className="mb-3 flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.24em] text-sky-300">
          <span>{item.media_label ?? "Título"}</span>
          {rating && (
        ...
        <h3 className={["line-clamp-2 max-w-[360px] tracking-tight text-white drop-shadow-[0_4px_18px_rgba(0,0,0,0.75)]", isLongTitle ? "text-[1.45rem] font-black leading-[1]" : "text-[1.75rem] font-black leading-[1.04]"].join(" ")}>
          {item.title_label}
        </h3>

        {item.genre_label && <p className="mt-3 text-[12px] font-semibold text-zinc-300">{item.genre_label}</p>}
        {item.overview && <p className="mt-3 line-clamp-4 max-w-[390px] text-[12px] leading-5 text-zinc-300">{item.overview}</p>}

        <p className="mt-3 line-clamp-1 text-[10px] font-bold text-sky-300">{item.reason ?? "Combina com você"}</p>

        <div className="mt-5">
          <span className="rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-xs font-bold text-white backdrop-blur-md transition group-hover:bg-white/15">
            Ver detalhes
          </span>
        </div>
      </div>
...
function SmallForYouCard({ item }: { item: ForYouItem }) {
  ...
  return (
    <article className="group relative h-[320px] overflow-hidden rounded-[1.35rem] border border-white/10 bg-white/[0.04] shadow-[0_16px_55px_rgba(0,0,0,0.38)] transition duration-300 hover:-translate-y-1 hover:border-sky-300/40 hover:shadow-[0_20px_75px_rgba(56,189,248,0.14)]">
      ...
      {image && (
        <Image src={image} alt={item.title_label} fill sizes="220px" className="object-cover transition duration-700 group-hover:scale-110" />
      )}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 p-4">
        ...
        <h3
          className={["min-h-[2.35rem] line-clamp-2 leading-tight tracking-tight text-white", item.title_label.length > 22 ? "text-[0.9rem] font-extrabold" : "text-[1.05rem] font-black"].join(" ")}
          title={item.title_label}
        >
          {item.title_label}
        </h3>
        ...
        <p className="mt-2 line-clamp-1 text-xs font-semibold text-zinc-300">
          {item.year ? `${item.year} · ` : ""}{item.media_label ?? "Título"}
        </p>

        <p className="mt-2 min-h-[1.65rem] line-clamp-2 text-[9px] font-bold leading-snug text-sky-300" title={item.reason ?? "Combina com você"}>
    ...
    <>
      <div className="-mx-6 flex gap-4 overflow-x-auto px-6 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-[260px] w-[180px] shrink-0 animate-pulse rounded-[1.35rem] bg-white/5" />
        ))}
      </div>
      <div className="hidden gap-5 lg:grid lg:grid-cols-[2.05fr_repeat(4,0.72fr)]">
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-[320px] animate-pulse rounded-[1.35rem] bg-white/5" />
        ))}
      </div>
    </>
  );
    ...
    <section>
      ...
      {loading ? (
        ...
        <>
          {/* Mobile */}
          <div className="-mx-6 flex gap-4 overflow-x-auto px-6 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden">
            {featured && <SmallForYouCard item={featured} />}
            {items.map((item) => (
