...
type FetchForYouFn = (titlesSnapshot: UserTitlesSnapshot, exclude: string[], mode?: "summary" | "full", silent?: boolean) => void;
...
function ForYouActions({ item, onDismiss }: {
    ...
    return (<div className="absolute right-2.5 top-2.5 z-40 flex gap-1.5">
...
// ─── FeaturedCard ─────────────────────────────────────────────────────────────
function FeaturedForYouCard({ item, onDismiss }: {
    item: ForYouItem;
    onDismiss?: () => void;
}) {
    ...
    return (<article className="group relative h-[320px] overflow-hidden rounded-[1.65rem] border border-white/10 bg-white/[0.04] shadow-[0_20px_80px_rgba(0,0,0,0.42)] transition duration-300 hover:-translate-y-1 hover:border-sky-300/40 hover:shadow-[0_24px_90px_rgba(56,189,248,0.16)]">
      ...
      {item.backdropUrl && (<div className="absolute inset-0 overflow-hidden">
          <div className="absolute inset-y-0 -left-[0%] right-50 w-[120%] aspect-video lg:aspect-auto">
            <TmdbImage path={item.backdropUrl} kind="backdrop" size="medium" alt={item.title} fill sizes="(max-width: 1024px) 100vw, 65vw" className="object-cover object-right"/>
          </div>
        </div>)}
      ...
      <div className={[
        ...
        <div className="mb-3 flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.24em] text-sky-300">
          ...
          {rating && (<span className="flex items-center gap-1 tracking-normal text-amber-400">
              <IconStar />
              {rating}
            </span>)}
          {item.best_provider_name && (<CardProviderBadge name={item.best_provider_name} logoPath={item.best_provider_logo} type={item.best_provider_type}/>)}
        </div>

        <LocalizedTitle as="h3" title={item.title} originalTitle={item.originalTitle} variant="large" className="max-w-[360px] drop-shadow-[0_4px_18px_rgba(0,0,0,0.75)] transition group-hover:text-sky-100"/>

        {item.genreLabel && (<p className="mt-2 text-[12px] font-semibold text-zinc-300">{item.genreLabel}</p>)}
...
// ─── SmallCard ────────────────────────────────────────────────────────────────
function SmallForYouCard({ item, onDismiss }: {
    item: ForYouItem;
    onDismiss?: () => void;
}) {
    ...
    return (<article className="group relative h-[320px] overflow-hidden rounded-[1.35rem] border border-white/10 bg-white/[0.04] shadow-[0_16px_55px_rgba(0,0,0,0.38)] transition duration-300 hover:-translate-y-1 hover:border-sky-300/40 hover:shadow-[0_20px_75px_rgba(56,189,248,0.14)]">
      ...
      {imagePath ? (<TmdbImage path={imagePath} kind={imageKind} size="card" alt={item.title} fill sizes="220px" className="object-cover transition duration-700 group-hover:scale-110"/>) : (
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 p-4">
        <div className="mb-2 flex items-center gap-1.5">
          {rating && (<div className="inline-flex items-center gap-[3px] rounded-full border border-white/[0.14] bg-black/75 px-2 py-[3px] text-[10px] font-semibold text-amber-400 backdrop-blur-[10px]">
              <IconStar />
              {rating}
            </div>)}
          {item.best_provider_name && (<CardProviderBadge name={item.best_provider_name} logoPath={item.best_provider_logo} type={item.best_provider_type}/>)}
...
function ForYouSkeleton() {
    return (<>
      <div className="no-scrollbar -mx-6 flex gap-4 overflow-x-auto px-6 pb-2 lg:hidden">
        {Array.from({ length: 4 }).map((_, index) => (<div key={index} className="h-[260px] w-[180px] shrink-0 animate-pulse rounded-[1.35rem] bg-white/5"/>))}
      </div>
      <div className="hidden gap-5 lg:grid lg:grid-cols-[2.05fr_repeat(5,0.72fr)]">
        ...
        {Array.from({ length: 5 }).map((_, index) => (<div key={index} className="h-[320px] animate-pulse rounded-[1.35rem] bg-white/5"/>))}
      </div>
    </>);
}
// ─── Seção principal ──────────────────────────────────────────────────────────
...
export default function ForYouSection() {
    ...
    const visiblePool = useMemo(() => {
        ...
        return all.filter((item) => {
            ...
            if (aliases.some((key) => libraryIdentities.has(key) || seen.has(key)))
                return false;
    ...
    const { ref: rowRef, canScrollLeft, canScrollRight, scrollLeft: doScrollLeft, scrollRight: doScrollRight, } = useScrollRow({ step: 392 });
    ...
    const fetchForYou = useCallback((titlesSnapshot: typeof titles, exclude: string[], mode: "summary" | "full" = "summary", silent = false) => {
        ...
        fetch("/api/user/for-you", {
            ...
            if (json) {
                ...
                if (json.featured)
                    newKeys.push(`${json.featured.mediaType}:${json.featured.id}`);
                for (const it of json.items ?? [])
                    newKeys.push(`${it.mediaType}:${it.id}`);
    ...
    useEffect(() => {
        if (titlesLoading)
            return;
        if (didInitRef.current && refreshCount === 0)
            return;
    ...
    if (!loading && visiblePool.length === 0)
        return null;
    return (<section>
      <SectionHeader title={uiMessage("ui.de4780f1e590")} subtitle={uiMessage("ui.f77da1519cfb")} className="mb-6" action={<div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            ].join(" ")} aria-label={uiMessage("ui.64c83ca67fb8")}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}>
                <path d="M1 4v6h6M23 20v-6h-6"/>
                <path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 0 1 3.51 15"/>
              </svg>{uiMessage("ui.075d4d12844d")}</button>
            <Link href="/para-voce" className="hidden text-xs font-bold text-sky-300 transition hover:text-white md:block">{uiMessage("ui.a817be3030ff")}</Link>
            <div className="flex lg:hidden">
              <ScrollRowArrows canScrollLeft={canScrollLeft} canScrollRight={canScrollRight} onLeft={doScrollLeft} onRight={doScrollRight}/>
            </div>
          </div>}/>

      {loading ? (<ForYouSkeleton />) : (<>
          <div ref={rowRef} className="no-scrollbar -mx-6 flex gap-4 overflow-x-auto px-6 pb-2 lg:hidden">
            {visibleFeatured && (<div className="w-[180px] shrink-0">
                <SmallForYouCard item={visibleFeatured} onDismiss={() => dismissItem(visibleFeatured.id, visibleFeatured.mediaType)}/>
              </div>)}
            {visibleItems.map((item) => (<div key={`${item.mediaType}-${item.id}`} className="w-[180px] shrink-0">
                <SmallForYouCard item={item} onDismiss={() => dismissItem(item.id, item.mediaType)}/>
              </div>))}
          </div>

          <div className="hidden gap-5 lg:grid" style={visibleItems.length > 0
                ? { gridTemplateColumns: `2.05fr repeat(${visibleItems.length}, 0.72fr)` }
                : { gridTemplateColumns: "1fr" }}>
            {visibleFeatured && (<FeaturedForYouCard item={visibleFeatured} onDismiss={() => dismissItem(visibleFeatured.id, visibleFeatured.mediaType)}/>)}
            {visibleItems.map((item) => (<SmallForYouCard key={`${item.mediaType}-${item.id}`} item={item} onDismiss={() => dismissItem(item.id, item.mediaType)}/>))}
          </div>
        </>)}
    </section>);
}
