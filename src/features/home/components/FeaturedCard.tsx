      ...
      <div className="w-full max-w-[500px] translate-y-4">
        ...
        <div className="grid grid-cols-[132px_1fr] gap-5">
          <div className="relative aspect-[2/3] overflow-hidden rounded-2xl bg-white/5 shadow-[0_18px_50px_rgba(0,0,0,0.55)]">
            {posterUrl && (<Image src={posterUrl} alt={title} fill priority sizes="132px" className="object-cover"/>)}
            <LibraryStateBadge tmdbId={item.id > 0 ? item.id : undefined} poplogId={item.poplogId} imdbId={item.externalIds?.imdbId} mediaType={mediaType}/>
          </div>

          <div className="min-w-0 pt-2">
            <LocalizedTitle as="h2" title={title} originalTitle={originalTitle} variant="large"/>
