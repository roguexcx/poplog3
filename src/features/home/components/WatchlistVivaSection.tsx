...
function selectFive(all: WatchlistTitle[]): Array<WatchlistTitle & {
    _slot: WatchlistSlot;
}> {
    ...
    const available = all.filter((t) => t.stream_status !== "cinemas" && t.stream_status !== "chegando");
    ...
    const picked: Array<WatchlistTitle & {
        _slot: WatchlistSlot;
    }> = [];
    ...
    while (picked.length < 5) {
        ...
        if (tryFridge) {
            if (!addFrom(fridge, "fridge"))
                addFrom(fillPool, "free");
        }
        else {
            if (!addFrom(fillPool, "free"))
                addFrom(fridge, "fridge");
        }
...
// ─── Badge config ─────────────────────────────────────────────────────────────
const STATUS_BADGE: Record<StreamStatus, {
    label: string;
    border: string;
    bg: string;
    text: string;
    ...
    confirmado: {
        label: uiMessage("ui.86ba1a7ce714"),
    ...
    cinemas: {
        label: uiMessage("ui.35ba0d43cb40"),
        border: "border-rose-400/40",
        bg: "bg-rose-500/[0.18]",
        text: "text-rose-300",
    },
    unavailable: {
        label: uiMessage("ui.e184042b6cb6"),
...
// ─── Icons ────────────────────────────────────────────────────────────────────
function IconTrash() {
    return (<svg viewBox="0 0 24 24" className="h-[13px] w-[13px]" fill="none" stroke="currentColor" strokeWidth={2}>
      <path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6"/>
    </svg>);
}
// ─── Card ─────────────────────────────────────────────────────────────────────
function WatchlistCard({ item, onDismiss, }: {
    item: WatchlistTitle & {
        _slot: WatchlistSlot;
    };
    ...
    return (<article className={[
      ...
      <div className="relative">
        <Link href={slug} className="block">
          <div className={[
            ...
            {posterPath && !imgErr ? (<CatalogImage src={posterPath} size="w342" alt={item.title} fill sizes="(max-width: 768px) 160px, 220px" className="object-cover brightness-[0.92] saturate-[1.05] transition-[transform,filter] duration-[600ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] group-hover:scale-[1.04] group-hover:brightness-100 group-hover:saturate-110" onError={() => setImgErr(true)}/>) : (<div className="flex h-full items-center justify-center bg-[#0e0e1a] text-[11px] text-[#3a3a55]">{uiMessage("ui.01c119dc9e1b")}</div>)}
            ...
            {mainProvider ? (<div className="absolute bottom-2.5 left-2.5 z-20 flex items-center gap-1.5 rounded-[5px] border border-white/[0.18] bg-black/[0.72] px-[7px] py-[3px] backdrop-blur-[8px]">
                <CatalogImage src={mainProvider.logo} size="w45" alt={mainProvider.name} width={13} height={13} className="rounded-[3px]"/>
        ...
        <div className={[
            ...
            {(item._slot === "recent" || item._slot === "old") && (<div className="absolute right-2.5 top-2.5 z-20 rounded-full border border-white/[0.18] bg-black/[0.82] px-2 py-[3px] backdrop-blur-[10px]">
        ...
        <div className="absolute left-2.5 top-2.5 z-30 flex flex-col gap-1.5 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <CardActionButton onClick={() => dismiss("watched")} disabled={false} title={uiMessage("ui.553f501bc5c7")} active={watchedActive} saving={false} activeClass="border-emerald-400/55 bg-emerald-400/[0.18] text-emerald-300 shadow-[0_0_10px_rgba(52,211,153,0.25)]">
            <IconCheck />
          </CardActionButton>
          <CardActionButton onClick={() => dismiss("remove")} disabled={false} title={uiMessage("ui.93851da121a8")} active={false} saving={false} activeClass="">
            <IconTrash />
          </CardActionButton>
        </div>
      </div>
      ...
      <Link href={slug} className="mt-2.5 block px-0.5">
        <LocalizedTitle as="h3" title={item.title} originalTitle={item.original_title_label} variant="poster" className="line-clamp-2 text-[13px] font-[500] leading-[1.35] tracking-[-0.01em] text-[#e8e8f0] transition-colors duration-200 group-hover:text-[#f4f4fa]"/>
        ...
        <p className="mt-1 flex flex-wrap items-center gap-[5px] text-[11px] text-[#52526a] truncate">
          ...
          <span>{isMovie ? "Filme" : item.seasons ? uiMessage("ui.f5f77c02c119", { v1: item.seasons }) : uiMessage("ui.74f19285073a")}</span>
          {item.runtime_label && (<>
...
export default function WatchlistVivaSection() {
    ...
    const [visible, setVisible] = useState<Array<WatchlistTitle & {
        _slot: WatchlistSlot;
    }>>([]);
    const [loading, setLoading] = useState(true);
    const { ref: rowRef, canScrollLeft, canScrollRight, scrollLeft: doScrollLeft, scrollRight: doScrollRight, } = useScrollRow({ step: 352 });
    ...
    function handleDismiss(item: WatchlistTitle, action: "watched" | "remove") {
        ...
        setVisible((prev) => {
            ...
            if (spare)
                return [...filtered, { ...spare, _slot: "free" as WatchlistSlot }];
            return filtered;
        });
    ...
    if (!loading && allTitles.length === 0)
        return null;
    return (<section>
      <SectionHeader title={uiMessage("ui.a82ce52d3e5c")} className="mb-6" action={<div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button onClick={reshuffle} disabled={loading || allTitles.length <= 5} className={[
            ...
            ].join(" ")} aria-label={uiMessage("ui.9db99fc86593")}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}>
                <path d="M1 4v6h6M23 20v-6h-6"/>
                <path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 0 1 3.51 15"/>
              </svg>{uiMessage("ui.7cde75a9ff0f")}</button>
            <div className="flex md:hidden">
              <ScrollRowArrows canScrollLeft={canScrollLeft} canScrollRight={canScrollRight} onLeft={doScrollLeft} onRight={doScrollRight}/>
            </div>
          </div>}/>

      <div ref={rowRef} className="no-scrollbar -mx-4 flex gap-4 overflow-x-auto px-4 pb-2 md:mx-0 md:grid md:grid-cols-5 md:overflow-visible md:px-0 md:pb-0">
        {loading
            ? Array.from({ length: 5 }).map((_, i) => (<div key={i} className="w-[160px] shrink-0 md:w-auto">
                <SkeletonCard />
              </div>))
            : visible.map((item) => (<div key={item.id} className="w-[160px] shrink-0 md:w-auto">
