...
const MOBILE_NAV_TRIGGERS = [
    {
        href: "/radar",
        label: uiMessage("ui.aad82db2cb77"),
        desc: "O que estreia",
        icon: Radar,
        tone: "sky",
    },
    {
        href: "#trending",
        label: uiMessage("ui.a43ecccd27a7"),
        desc: "Em alta agora",
        icon: Sparkles,
        tone: "violet",
    },
    {
        ...
        label: "Filmes",
        desc: uiMessage("ui.b41f9ffd0e27"),
        icon: Film,
        tone: "emerald",
    },
    {
        href: "/buscar?type=tv",
        label: uiMessage("ui.de212174bc0c"),
        desc: uiMessage("ui.b41f9ffd0e27"),
        icon: Tv,
        tone: "amber",
    },
...
export default function HeroSection({ backdropUrl, featuredItem, featuredType, featuredTitle, featuredRank, featuredTypeLabel, posterUrl, year, runtime, seasons, genres, overview, }: Props) {
    ...
    return (<section className="-mx-4 -mt-4 relative z-30 min-h-[66vh] overflow-visible sm:-mx-6 md:-mx-8 md:-mt-6 lg:-mx-10">
      ...
      <div className="relative z-10 mx-auto grid min-h-[66vh] max-w-[1560px] items-center gap-10 px-6 pb-16 pt-8 md:px-10 lg:grid-cols-[1fr_500px]">
        <div className="max-w-3xl">
          <p className="mb-7 text-[11px] font-bold uppercase leading-5 tracking-[0.38em] text-zinc-300">{uiMessage("ui.cd435800aca8")}<br />{uiMessage("ui.b5efcd17e77d")}</p>
        ...
        {featuredItem && (<FeaturedCard item={featuredItem} mediaType={featuredType} title={featuredTitle} featuredRank={featuredRank} featuredTypeLabel={featuredTypeLabel} posterUrl={posterUrl} year={year} runtime={runtime} seasons={seasons} genres={genres} overview={overview}/>)}
      </div>
    </section>);
}
