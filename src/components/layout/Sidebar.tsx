...
type UiTranslator = (key: string, replacements?: Record<string, string | number | null | undefined>) => string;
...
function buildNavigation(ui: UiTranslator, language: keyof typeof SIDEBAR_COPY) {
    const copy = SIDEBAR_COPY[language];
    const navGroups: NavGroup[] = [
        {
            label: copy.discover,
            links: [
                { href: "/", label: ui("ui.fc2c74003cb1"), icon: Home },
                { href: "/buscar", label: ui("ui.a62405e2cefb"), icon: Search },
            ],
        },
        {
            label: copy.follow,
            links: [
                { href: "/acompanhando", label: copy.watching, icon: PlayCircle, authRequired: true },
                ...(FEATURES.RADAR ? [{ href: "/radar", label: ui("ui.aad82db2cb77"), icon: Radar } as NavLink] : []),
            ],
        },
        {
            label: copy.organize,
            links: [
                { href: "/library", label: ui("ui.497ac5c11bf3"), icon: Library, authRequired: true },
            ],
        },
        {
            label: copy.explore,
            links: [
                { href: "/sorteio", label: ui("ui.45a25136ea07"), icon: Dice5, authRequired: true },
            ],
        },
        {
            label: copy.account,
            links: [
                { href: "/profile", label: copy.profile, icon: UserCircle, authRequired: true },
                { href: "/admin", label: ui("admin.nav.label"), icon: Shield, authRequired: true, adminRequired: true },
            ],
        },
    ];
    ...
    const mobileMoreLinks: NavLink[] = [
        ...(FEATURES.RADAR ? [{ href: "/radar", label: ui("ui.aad82db2cb77"), icon: Radar } as NavLink] : []),
        { href: "/buscar?type=movie", label: copy.movies, icon: Film },
        { href: "/buscar?type=tv", label: ui("ui.de212174bc0c"), icon: Tv },
        { href: "/sorteio", label: ui("ui.45a25136ea07"), icon: Dice5, authRequired: true },
        { href: "/profile", label: copy.profile, icon: UserCircle, authRequired: true },
        { href: "/admin", label: ui("admin.nav.label"), icon: Shield, authRequired: true, adminRequired: true },
    ];

    return { copy, navGroups, mobileLinks, mobileMoreLinks };
}
export default function Sidebar() {
    const pathname = usePathname();
    const router = useRouter();
    const { locale, ui } = useLocale();
    const { copy, navGroups, mobileLinks, mobileMoreLinks } = useMemo(
        () => buildNavigation(ui, locale.interfaceLanguage),
        [locale.interfaceLanguage, ui],
    );
    ...
    return (<>
      <aside onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={`peer/sidebar fixed left-0 top-0 z-50 hidden h-screen flex-col border-r border-white/5 bg-[#09090f]/85 backdrop-blur-xl transition-all duration-300 ease-in-out md:flex ${isHovered ? "w-64" : "w-20"}`}>
        <div className="flex items-center gap-4 px-5 py-5">
          <Link href="/" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 to-purple-700 shadow-lg shadow-indigo-500/20" aria-label={ui("ui.cc85e897e2a5")}>
          ...
          {user ? (<button type="button" onClick={handleLogout} className="group flex w-full items-center gap-3 rounded-xl p-2 text-left text-zinc-500 transition hover:bg-white/5 hover:text-white">
              ...
              <div className={`min-w-0 transition-all duration-300 ${isHovered
                ...
                <p className="flex items-center gap-1 truncate text-[10px] text-zinc-500">
                  <LogOut size={12}/>{ui("ui.3b79237730c4")}</p>
              </div>
              ...
              <div className={`min-w-0 transition-all duration-300 ${isHovered
                ? "translate-x-0 opacity-100"
                : "pointer-events-none -translate-x-2 opacity-0"}`}>
                <p className="truncate text-sm font-semibold text-white">{ui("ui.7573ba85155c")}</p>

                <p className="truncate text-[10px] text-zinc-500">{ui("ui.9b63d74160ce")}</p>
              </div>
            </button>)}
        </div>
      </aside>
      ...
      <nav className="fixed bottom-0 left-0 right-[calc(100vw-100%)] z-50 border-t border-white/5 bg-[#09090f]/90 px-1.5 pb-[calc(env(safe-area-inset-bottom,0px)+0.35rem)] pt-1.5 backdrop-blur-xl md:hidden">
        <div className="grid grid-cols-5 items-center">
          {mobileLinks.filter(isVisible).map((link) => {
            const active = isActive(link.href);
      ...
      {mobileMenuOpen && (<div className="fixed inset-0 z-[60] md:hidden">
          <button type="button" aria-label={ui("ui.9da31443718f")} onClick={() => setMobileMenuOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-sm"/>
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-3xl border-t border-white/10 bg-[#09090f] p-4 pb-[calc(env(safe-area-inset-bottom,0px)+1rem)] shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-base font-bold text-white">{ui("ui.8f453e63fe4d")}</p>
                <p className="text-xs text-zinc-500">{ui("ui.3c2862f24d9f")}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {mobileMoreLinks.filter(isVisible).map((link) => {
                const active = isActive(link.href);
              ...
              {user ? (<button type="button" onClick={handleLogout} className="flex w-full items-center gap-3 rounded-2xl bg-white/[0.03] p-3 text-left text-zinc-300 transition hover:bg-white/[0.06] hover:text-white">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="flex items-center gap-1 text-xs text-zinc-500">
                      <LogOut size={13}/>{ui("ui.3b79237730c4")}</p>
                  </div>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white">{ui("ui.7573ba85155c")}</p>
                    <p className="text-xs text-zinc-500">{ui("ui.9b63d74160ce")}</p>
                  </div>
                </button>)}
            </div>
          </div>
