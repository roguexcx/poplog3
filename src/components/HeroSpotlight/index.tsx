...
function HeroOnboarding() {
  return (
    <div className="flex h-full min-h-[420px] flex-col items-center justify-center gap-4 rounded-[28px] border border-white/[0.08] bg-white/[0.02] px-8 text-center">
      ...
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-white/30">Nada por aqui ainda</p>
        <h3 className="mt-2 text-xl font-black tracking-[-0.03em] text-white/70">Adicione séries e filmes</h3>
        <p className="mt-2 text-sm text-white/35">Sua vitrine pessoal aparece assim que você começar a acompanhar algum título.</p>
      </div>
    </div>
  );
}
...
function HeroPosterCluster({
  ...
  return (
    <div className="pointer-events-none absolute right-12 top-1/2 z-20 hidden -translate-y-1/2 xl:block group-hover:translate-x-2 transition-transform duration-500">
      <div className="relative h-[390px] w-[260px]">
        {stackItems.map((item, index) => {
          ...
          return (
            <button
              ...
              className="pointer-events-auto absolute right-0 top-0 overflow-hidden rounded-[22px] border border-white/[0.08] bg-white/[0.035] opacity-40 shadow-2xl transition-all duration-500 hover:opacity-100 hover:scale-[1.03] hover:blur-0 blur-[0.4px]"
              style={{
                width: 145,
        ...
        <button
          ...
          className="pointer-events-auto absolute right-6 top-6 z-10 overflow-hidden rounded-[28px] border border-white/[0.18] bg-white/[0.055] shadow-[0_35px_100px_rgba(0,0,0,0.65)] backdrop-blur-xl transition-all duration-500 hover:-translate-y-1.5 hover:scale-[1.02] hover:border-white/[0.35]"
          style={{ width: 195 }}
        >
          <div className="relative aspect-[2/3] overflow-hidden">
            <TmdbImage
...
export default function HeroSpotlight({
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      if (total === 0) return;
      if (event.key === "ArrowRight") dispatch({ type: "advance" });
      if (event.key === "ArrowLeft") dispatch({ type: "prev" });
    }
  ...
  return (
    <div
      className="group relative h-[430px] w-full overflow-hidden rounded-[24px] max-[360px]:h-[455px] sm:h-[clamp(460px,46vw,620px)] sm:rounded-[28px]"
    ...
    >
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          {item ? (
            <HeroSlide item={item} onNotNow={handleNotNow} onCTAClick={handleCTAClick} onNavigate={handleBannerClick} />
          ) : null}
        </motion.div>
      </AnimatePresence>

      {item ? (
        <HeroPosterCluster currentItem={item} items={items} currentIndex={current} onNavigate={onNavigate} />
      ) : null}
      ...
      {total > 1 && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 z-30 hidden -translate-y-1/2 rounded-full border border-white/10 bg-black/40 p-3 text-white/50 opacity-0 backdrop-blur-md transition-all duration-300 hover:bg-black/60 hover:text-white group-hover:opacity-100 md:flex"
          ...
          <button
            ...
            className="absolute right-4 top-1/2 z-30 hidden -translate-y-1/2 rounded-full border border-white/10 bg-black/40 p-3 text-white/50 opacity-0 backdrop-blur-md transition-all duration-300 hover:bg-black/60 hover:text-white group-hover:opacity-100 md:flex"
      ...
      {total > 1 ? (
        <div className="pointer-events-auto absolute bottom-4 left-1/2 z-20 -translate-x-1/2 md:bottom-6 md:left-8 md:translate-x-0">
