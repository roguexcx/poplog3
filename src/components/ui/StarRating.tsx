...
const SIZE_MAP: Record<StarRatingSize, { star: string; gap: string; label: string }> = {
    ...
    >
      <defs>
        <linearGradient id={`half-${fill}`} x1="0" x2="1" y1="0" y2="0">
          <stop
            offset="50%"
            stopColor={fill === "empty" ? "currentColor" : "rgb(250 204 21)"}
            stopOpacity={fill === "empty" ? "0.25" : "1"}
          />
          <stop
            offset="50%"
...
export default function StarRating({
  ...
  function starFill(index: number): "full" | "half" | "empty" {
    ...
    if (activeValue >= pos) return "full";
    if (activeValue >= pos - 0.5) return "half";
    return "empty";
  }
  ...
  return (
    <div
      ...
      aria-label={ariaLabel ?? `Avaliação: ${value ?? 0} de ${max} estrelas`}
    ...
    >
      {stars.map((i) =>
        interactive ? (
          <button
            key={i}
            type="button"
            className={`${star} relative cursor-pointer text-white/40 transition-transform duration-75 hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400 focus-visible:ring-offset-1 focus-visible:ring-offset-transparent`}
          ...
          <span key={i} className={`${star} text-white/40`}>
            <StarSvg fill={starFill(i)} className="h-full w-full" />
          </span>
        )
      )}
      ...
      {showLabel && value !== null && value !== undefined && (
        <span
          className={`${label} ml-1 font-bold tabular-nums text-white/70`}
        >
          {value.toFixed(1)}
        </span>
      )}
