...
export default function LoginDrawer({ open, onClose, oauthAvailable = false, }: LoginDrawerProps) {
    ...
    async function handleGoogleSignIn() {
        if (!oauthAvailable) {
            setError(uiMessage("ui.3a535ca93078") +
    ...
    return (<>
      <div ref={overlayRef} onClick={onClose} className={`
          fixed inset-0 z-40
        ...
        <div className="flex h-full flex-col overflow-y-auto px-8 py-10">
          ...
          <div className="mb-10 mt-4 md:mt-0">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-indigo-400">{uiMessage("ui.d78bff23f629")}</span>
            </div>

            <h2 className="text-3xl font-semibold tracking-tight text-white">{uiMessage("ui.26d1e8cbd5b0")}</h2>

            <p className="mt-3 text-sm leading-relaxed text-zinc-500">{uiMessage("ui.f9b84cd395c7")}</p>
          </div>

          <button type="button" onClick={handleGoogleSignIn} disabled={oauthLoading} className="
