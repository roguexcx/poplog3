...
export default function ApiDebugClient() {
    ...
    const [apis, setApis] = useState<ApiState[]>(APIS.map((api) => ({ ...api, loading: false })));
      ...
      <div className="mx-auto max-w-7xl">
        <header className="border-b border-white/10 pb-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-indigo-300/80">{uiMessage("ui.275a3916b059")}</p>
              <h1 className="mt-3 text-3xl font-black text-white sm:text-5xl">{uiMessage("ui.53a2b1cbc1f0")}</h1>
              <p className="mt-4 max-w-3xl text-sm leading-6 text-zinc-400 sm:text-base">{uiMessage("ui.220a008d3e08")}</p>
            </div>

            <button type="button" onClick={() => void loadAll()} disabled={!secret.trim()} className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-indigo-400/30 bg-indigo-500/10 px-4 text-sm font-bold text-indigo-100 transition hover:bg-indigo-500/20">
              <RefreshCw size={17}/>{uiMessage("ui.93a31e6fb94b")}</button>
          </div>

          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:items-center">
            <label className="shrink-0 text-xs font-semibold uppercase tracking-widest text-zinc-500 sm:w-28">{uiMessage("ui.3cefe204cebd")}</label>
            <input type="password" placeholder="ADMIN_SECRET" value={secret} onChange={(event) => setSecret(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void loadAll()} className="h-10 w-full max-w-sm rounded-lg border border-white/10 bg-white/[0.06] px-3 text-sm text-white placeholder-zinc-500 outline-none focus:border-indigo-400/40 focus:ring-1 focus:ring-indigo-400/20"/>
          </div>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Metric label={uiMessage("ui.0748e2f5bdcf")} value={`${metrics.errors}`} detail="falha ou chave ausente" tone={metrics.errors ? "bad" : "good"}/>
...
function ApiStatusCard({ api }: {
    ...
    return (<div className={`rounded-lg border bg-white/[0.03] p-4 ${tone}`}>
...
function ApiBlock({ api }: {
    ...
    if (api.loading) {
        return (<section className="rounded-lg border border-white/10 bg-white/[0.03] p-6">
        <h2 className="text-xl font-black text-white">{api.label}</h2>
        <p className="mt-2 text-sm text-zinc-400">{uiMessage("ui.e86577f48928")}</p>
      </section>);
    }
    if (!api.data) {
        return (<section className="rounded-lg border border-rose-300/20 bg-rose-500/[0.04] p-6">
        <h2 className="text-xl font-black text-white">{api.label}</h2>
        <p className="mt-2 text-sm text-rose-200">{api.error ?? "Falha ao carregar."}</p>
      </section>);
    }
    const data = api.data;
    return (<section className="rounded-lg border border-white/10 bg-[#09090f]/70 p-5 shadow-2xl shadow-black/20 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        <InfoPanel title={uiMessage("ui.9f8a9882c1e4")} items={data.summary.bestFor}/>
        <InfoPanel title={uiMessage("ui.509c4c1a0ccc")} items={data.summary.dataTypes}/>
        <InfoPanel title={uiMessage("ui.2de6b54415ae")} items={data.capabilities}/>
      </div>

      <div className="mt-6 space-y-6">
        {data.sections.map((section) => (<DebugSectionView key={section.title} section={section}/>))}
      </div>

      <details className="mt-6 rounded-lg border border-white/10 bg-black/20 p-4">
        <summary className="cursor-pointer text-sm font-bold text-zinc-200">{uiMessage("ui.0dda5f4a8f73")}</summary>
        <div className="mt-4 space-y-4">
          {data.endpoints.map((endpoint) => (<details key={`${endpoint.label}-${endpoint.url}`} className="rounded-lg border border-white/10 bg-black/20 p-3">
      ...
      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.025] p-4">
        <h3 className="text-sm font-black uppercase tracking-[0.18em] text-zinc-400">{uiMessage("ui.2a6cd37db731")}</h3>
        <ul className="mt-3 space-y-2 text-sm leading-6 text-zinc-300">
          {data.observations.map((observation) => (<li key={observation}>- {observation}</li>))}
        </ul>
      </div>
    </section>);
}
function InfoPanel({ title, items }: {
    ...
    return (<div className="rounded-lg border border-white/10 bg-white/[0.025] p-4">
...
function DebugSectionView({ section }: {
    section: DebugSection;
}) {
    return (<details open className="rounded-lg border border-white/10 bg-white/[0.025] p-4">
      ...
      {section.bullets && section.bullets.length > 0 && (<ul className="mt-4 grid gap-2 text-sm text-zinc-300 sm:grid-cols-2">
          {section.bullets.map((bullet) => <li key={bullet}>- {bullet}</li>)}
        </ul>)}

      {section.cards && section.cards.length > 0 && (<div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {section.cards.map((card, index) => <MovieDebugCard key={`${card.title}-${index}`} card={card}/>)}
        </div>)}

      {section.tables && section.tables.length > 0 && (<div className="mt-4 grid gap-4">
          {section.tables.map((table) => <DebugTableView key={table.title} table={table}/>)}
        </div>)}
    </details>);
}
function MovieDebugCard({ card }: {
    card: DebugCard;
}) {
    return (<article className="overflow-hidden rounded-lg border border-white/10 bg-black/20">
      <div className="flex gap-3 p-3">
        <div className="h-28 w-20 shrink-0 overflow-hidden rounded-md bg-white/[0.06]">
          {card.image ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={card.image} alt="" className="h-full w-full object-cover"/>) : (<div className="grid h-full place-items-center text-xs font-bold text-zinc-600">{uiMessage("ui.9e63d4cc70a1")}</div>)}
        </div>
      ...
      {card.meta && card.meta.length > 0 && (<div className="border-t border-white/10 px-3 py-2 text-[11px] leading-5 text-zinc-500">
          {card.meta.slice(0, 4).join(" | ")}
        </div>)}
    </article>);
}
function DebugTableView({ table }: {
    table: DebugTable;
}) {
    return (<div className="overflow-hidden rounded-lg border border-white/10">
      ...
      <div className="overflow-auto">
        <table className="min-w-full text-left text-xs">
          <thead className="bg-black/20 text-zinc-500">
            <tr>
              {table.columns.map((column) => (<th key={column} className="px-3 py-2 font-bold uppercase tracking-[0.14em]">{column}</th>))}
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5 text-zinc-300">
            {table.rows.length ? table.rows.map((row, index) => (<tr key={`${table.title}-${index}`}>
                {row.map((cell, cellIndex) => (<td key={`${table.title}-${index}-${cellIndex}`} className="max-w-[22rem] px-3 py-2 align-top">
                    {cell}
                  </td>))}
              </tr>)) : (<tr>
                <td className="px-3 py-3 text-zinc-500" colSpan={table.columns.length}>{uiMessage("ui.ea4cf25ae9c7")}</td>
              </tr>)}
          </tbody>
        </table>
      </div>
...
function Badge({ children, tone = "neutral", }: {
    ...
    return (<span className={`inline-flex items-center rounded-md border px-2 py-1 text-[11px] font-bold leading-none ${toneClass}`}>
      {children}
    </span>);
}
