...
import { orderCatalogTitlesByLanguage } from "@/lib/i18n/catalog-localization";
...
async function buildLocalWatchlistPicks(
  ...
  try {
    ...
    const cached = !refreshCache
      ? await readContinuitySectionCache<WatchlistPicksCachePayload>(sectionKey, {
          userId,
      ...
      return {
        ...
        title: (() => {
          ...
          return resolveDisplayTitle({
            title: ordered.primary,
            originalTitle: ordered.secondary,
...
export async function GET(request: NextRequest) {
  try {
    ...
    return buildLocalWatchlistPicks(user.id, request, scope);
  } catch (err) {
