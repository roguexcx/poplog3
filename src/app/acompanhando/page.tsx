...
import NewEpisodeCard, { type NewEpisodeItem, } from "@/features/acompanhando/NewEpisodeCard";
import ContinueCard, { type ContinueItem, } from "@/features/acompanhando/ContinueCard";
import WatchlistPickCard, { type WatchlistPickItem, } from "@/features/acompanhando/WatchlistPickCard";
import StartSeriesBanner from "@/features/acompanhando/StartSeriesBanner";
import RecentlyWatchedCard, { type RecentlyWatchedItem, } from "@/features/acompanhando/RecentlyWatchedCard";
...
async function fetchJsonSafe<T>(url: string): Promise<FetchResult<T>> {
    ...
    if (!res.ok) {
        console.warn(uiMessage("ui.0dd0a0332779"), {
            url,
            status: res.status,
            contentType,
        });
    ...
    if (!contentType.includes("application/json")) {
        ...
        return { ok: false, status: res.status, reason: uiMessage("ui.d5dd638b81b3") };
    }
    try {
        ...
        const reason = err instanceof Error ? err.message : uiMessage("ui.0b06d615a836");
...
function SectionPagination({ page, totalPages, onPageChange, }: {
    ...
    return (<div className="flex items-center justify-center gap-1.5 pt-2">
      <button type="button" disabled={page === 1} onClick={() => onPageChange(page - 1)} className="h-8 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 text-xs font-black text-white/45 transition hover:border-white/[0.15] hover:text-white disabled:cursor-not-allowed disabled:opacity-25">
        ‹
      </button>
      ...
      <button type="button" disabled={page === totalPages} onClick={() => onPageChange(page + 1)} className="h-8 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 text-xs font-black text-white/45 transition hover:border-white/[0.15] hover:text-white disabled:cursor-not-allowed disabled:opacity-25">
        ›
      </button>
    </div>);
}
...
export default function AcompanhandoPage() {
    ...
    useEffect(() => {
        fetchJsonSafe<{
            ...
            if (result.ok) {
                ...
                console.error(uiMessage("ui.4538b3cf48d8"), result.reason);
            }
        })
            .catch((err) => {
            setNewEpisodesError(err instanceof Error ? err.message : String(err));
            console.error(uiMessage("ui.ab8e09d69f45"), err);
        })
            .finally(() => {
    ...
    useEffect(() => {
        fetchJsonSafe<{
            ...
            .catch((err) => {
            console.error(uiMessage("ui.69308de7edf2"), err);
        })
            .finally(() => {
    ...
    const fetchWatchlistPicks = useCallback((excludeIds: string[] = []) => {
        ...
        const allExcludeIds = Array.from(new Set([...excludeIds, ...Array.from(shownStartSeriesIds.current)]));
        ...
        fetchJsonOrNull<{
            ...
            if (data && Array.isArray(data.items)) {
                ...
                data.items.forEach((item: WatchlistPickItem) => shownWatchlistIds.current.add(item.content_id));
            }
        })
            .catch((err) => {
    ...
    const fetchStartSeriesPicks = useCallback((excludeIds: string[] = []) => {
        ...
        fetchJsonOrNull<{
            ...
            if (data && Array.isArray(data.items)) {
                ...
                data.items.forEach((item: WatchlistPickItem) => shownStartSeriesIds.current.add(item.content_id));
            }
        })
            .catch((err) => {
            console.error(uiMessage("ui.fb83e3680246"), err);
        })
            .finally(() => {
    ...
    const snoozeItem = useCallback(async (contentId: string, durationHours = 4) => {
    ...
    const logSignal = useCallback(async (contentId: string, signal: SignalType, value?: object) => {
        await postCuradoriaAction({
            action: "signal",
            content_id: contentId,
            signal,
    ...
    const sortedContinueItems = useMemo(() => {
        if (continueSortMode === "season") {
            return [...continueItems].sort((a, b) => {
                ...
                return (a.episodes_behind - b.episodes_behind ||
                    (b.last_watched_at ?? "").localeCompare(a.last_watched_at ?? ""));
            });
        }
    ...
    return (<PageShell variant="wide">
      <div className="flex flex-col gap-10">
        {/* Hero */}
        {isHeroLoading ? (<div className="h-[480px] animate-pulse rounded-[28px] bg-white/[0.03]"/>) : (<HeroSpotlight items={heroItems} onSnooze={snoozeItem} onLogSignal={logSignal} onNavigate={handleNavigate}/>)}

        {/* Novos episódios esperando */}
        {(isNewEpisodesLoading || newEpisodeItems.length > 0 || newEpisodesError !== null) && (<section>
            <SectionHeader eyebrow={uiMessage("ui.62ada494dc46")} accent="rose" title={uiMessage("ui.fd17514822f3")} size="sm" action={!isNewEpisodesLoading && newEpisodeItems.length > 0 ? (<span className="rounded-full border border-white/[0.10] bg-white/[0.05] px-3 py-1 text-[11px] font-bold text-white/50">
                    {newEpisodeItems.length}{" "}
                    {newEpisodeItems.length === 1 ? uiMessage("ui.8a1be36de69d") : uiMessage("ui.d3e75bbc154a")}
                  </span>) : undefined} className="mb-4"/>

            {isNewEpisodesLoading ? (<div className="grid gap-2.5 md:grid-cols-2">
                {[...Array(4)].map((_, i) => (<div key={i} className="h-[76px] animate-pulse rounded-2xl bg-white/[0.04]"/>))}
              </div>) : newEpisodesError !== null ? (<p className="py-3 text-xs text-white/30">{uiMessage("ui.29da66cb15f8")}</p>) : (<div className="grid gap-2.5 md:grid-cols-2">
                {newEpisodeItems.map((item) => (<NewEpisodeCard key={item.content_id} item={item} onClick={() => handleNewEpisodeNavigate(item)}/>))}
              </div>)}
          </section>)}

        {/* Continue de onde parou */}
        {(isContinueLoading || continueItems.length > 0 || continueError !== null) && (<section>
            <SectionHeader eyebrow={uiMessage("ui.5ca3739c28b7")} accent="indigo" title={uiMessage("ui.6bf740d3af70")} size="sm" action={<div className="flex items-center gap-3">
                  {!isContinueLoading && continueItems.length > 0 && (<span className="rounded-full border border-white/[0.10] bg-white/[0.05] px-3 py-1 text-[11px] font-bold text-white/50">
                      {continueItems.length}{" "}
                      {continueItems.length === 1 ? uiMessage("ui.8a1be36de69d") : uiMessage("ui.d3e75bbc154a")}
                    </span>)}
                  <div className="flex rounded-full border border-white/[0.10] bg-white/[0.04] p-0.5">
                    <button type="button" onClick={() => setContinueSortMode("recent")} className={[
                    ...
                    <button type="button" onClick={() => setContinueSortMode("season")} className={[
                    "rounded-full px-3 py-1 text-[11px] font-bold transition-all",
                    continueSortMode === "season"
                        ? "bg-white text-zinc-900"
                        : "text-white/45 hover:text-white/70",
                ].join(" ")}>{uiMessage("ui.bbce6d2d298a")}</button>
                    <button type="button" onClick={() => setContinueSortMode("series")} className={[
                    "rounded-full px-3 py-1 text-[11px] font-bold transition-all",
                    continueSortMode === "series"
                        ? "bg-white text-zinc-900"
                        : "text-white/45 hover:text-white/70",
                ].join(" ")}>{uiMessage("ui.f544e973085a")}</button>
                  </div>
                </div>} className="mb-4"/>

            {isContinueLoading ? (<div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-3">
                {[...Array(continueItemsPerPage)].map((_, i) => (<div key={i} className="h-[95px] animate-pulse rounded-2xl bg-white/[0.04]"/>))}
              </div>) : continueError !== null ? (<p className="py-3 text-xs text-white/30">{uiMessage("ui.86ea2f9b5ef9")}</p>) : (<div className="flex flex-col gap-4">
                <div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-3">
                  {visibleContinueItems.map((item) => (<ContinueCard key={item.content_id} item={item} onClick={() => handleContinueNavigate(item)}/>))}
                </div>
                <SectionPagination page={safeContinuePage} totalPages={continueTotalPages} onPageChange={setContinuePage}/>
              </div>)}
          </section>)}

        {/* Da sua watchlist */}
        {(isWatchlistPicksLoading || watchlistPicks.length > 0) && (<section>
            <SectionHeader eyebrow={uiMessage("ui.a82ce52d3e5c")} accent="cyan" title={uiMessage("ui.516fbde88a2e")} size="sm" action={<button type="button" onClick={refreshWatchlistPicks} disabled={isWatchlistPicksLoading} className="h-8 rounded-full border border-white/[0.10] bg-white/[0.04] px-3 text-[11px] font-bold text-white/45 transition-all hover:border-white/[0.16] hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-40">{uiMessage("ui.1812e1ea46ab")}</button>} className="mb-5 sm:items-center"/>

            {isWatchlistPicksLoading ? (<div className="grid grid-cols-2 gap-x-4 gap-y-5 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-9">
                {[...Array(9)].map((_, i) => (<div key={i} className={[
                        "aspect-[2/3] animate-pulse rounded-xl bg-white/[0.04]",
                        i >= 8 ? "hidden sm:block" : "",
                    ].join(" ")}/>))}
              </div>) : (<div className="grid grid-cols-2 gap-x-4 gap-y-5 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-9">
                {watchlistPicks.map((item, index) => (<div key={item.content_id} className={index >= 8 ? "hidden sm:block" : ""}>
                    <WatchlistPickCard item={item} onClick={() => handleWatchlistPickNavigate(item)}/>
                  </div>))}
              </div>)}
          </section>)}

        {/* Últimos vistos */}
        {(isRecentlyWatchedLoading || recentlyWatched.length > 0) && (<section>
            <SectionHeader eyebrow={uiMessage("ui.bea29e92ac2b")} accent="neutral" title={uiMessage("ui.2d649ba13949")} size="sm" className="mb-4"/>

            {isRecentlyWatchedLoading ? (<div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-3">
                {[...Array(6)].map((_, i) => (<div key={i} className={[
                        "h-[72px] animate-pulse rounded-xl bg-white/[0.04] sm:h-[68px]",
                        i >= 3 ? "hidden sm:block" : "",
                    ].join(" ")}/>))}
              </div>) : (<div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-3">
                {recentlyWatched.map((item, index) => (<div key={item.content_id} className={index >= 3 ? "hidden sm:block" : ""}>
                    <RecentlyWatchedCard item={item} onClick={() => handleRecentlyWatchedNavigate(item)}/>
                  </div>))}
              </div>)}
          </section>)}

        {/* Boa hora pra começar */}
        {(isStartSeriesLoading || startSeriesPicks.length > 0) && (<section>
            <SectionHeader eyebrow={uiMessage("ui.a82ce52d3e5c")} accent="amber" title={uiMessage("ui.f5ae3da969d5")} subtitle={uiMessage("ui.3199efe9db09")} size="sm" action={<button type="button" onClick={refreshStartSeriesPicks} disabled={isStartSeriesLoading} className="h-8 rounded-full border border-white/[0.10] bg-white/[0.04] px-3 text-[11px] font-bold text-white/45 transition-all hover:border-white/[0.16] hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-40">
                  Trocar
                </button>} className="mb-4 sm:items-start"/>

            <StartSeriesBanner items={startSeriesPicks} loading={isStartSeriesLoading} onPick={handleStartSeriesNavigate}/>
          </section>)}
      </div>
    </PageShell>);
}
