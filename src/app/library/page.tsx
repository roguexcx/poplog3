...
export const metadata: Metadata = {
    title: uiMessage("ui.497ac5c11bf3"),
};
import LibraryPageView from "@/features/library/LibraryPage";
import { getCurrentUser } from "@/server/auth/get-current-user";
import { getUserLibrary, type Poplog3UserLibraryItem, } from "@/server/library/library-service";
import { getUserListMembership, getUserListSummaries, } from "@/server/lists/list-service";
...
import { buildTraktScoreMapFromIndex, serializeTraktScoreMap, } from "@/lib/score/library-popularity";
...
export default async function LibraryPage({ searchParams, }: LibraryPageProps) {
    ...
    if (user) {
        for (let offset = 0; offset < library.length; offset += 100) {
            ...
            for (const [key, listIds] of chunk)
                membership[key] = listIds;
        }
    }
    ...
    return (<PageShell variant="wide">
      <LibraryPageView key={initialTab ?? "all"} library={library} initialTab={initialTab} traktScores={traktScores} initialLists={lists} listMembership={membership} isAuthenticated={Boolean(user)}/>
    </PageShell>);
}
