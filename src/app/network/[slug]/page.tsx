...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: name,
        description: uiMessage("ui.16216325dc6b", { v1: name }),
    };
}
export default async function NetworkPage({ params }: Props) {
    ...
    return (<NetworkPageClient slug={canonicalSlug} displayName={networkDisplayName(canonicalSlug)} initialTitles={titles} initialTotal={total} pageSize={PAGE_SIZE}/>);
}
