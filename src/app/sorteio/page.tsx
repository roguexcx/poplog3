...
import { ArrowRight, Bookmark, Check, Clapperboard, Eye, EyeOff, Film, RotateCcw, Shuffle, SlidersHorizontal, Sparkles, Tv, } from "lucide-react";
...
const GENRES: Record<number, string> = {
    28: uiMessage("ui.d621dc82376b"),
    12: "Aventura",
    16: uiMessage("ui.fad2415d4676"),
    35: uiMessage("ui.aef1b13b3134"),
    80: "Crime",
    99: uiMessage("ui.bee226d85f29"),
    18: "Drama",
    10751: uiMessage("ui.e5db797bd446"),
    14: "Fantasia",
    27: "Terror",
    9648: uiMessage("ui.37afbcb110c7"),
    10749: "Romance",
    878: uiMessage("ui.5988e9350caf"),
    53: "Suspense",
    10752: "Guerra",
    10759: uiMessage("ui.374a26c2f319"),
    10765: uiMessage("ui.5c6aa7681a3a"),
    36: uiMessage("ui.da3160f7e8cd"),
};
...
function libraryLabel(item: AgendaItem) {
    ...
    switch (state) {
        ...
        case "completed":
        case "watched":
            return uiMessage("ui.b6c55536b2f9");
        case "abandoned":
            return "Abandonado";
        case "fridge":
            return "Na geladeira";
...
function contextMessage(item: AgendaItem): string {
    ...
    const providerName = getCanonicalProviderDisplayName({ name: item.best_provider_name }) ?? item.best_provider_name;
    if (item.is_preferred_provider && item.best_provider_name) {
        return uiMessage("ui.97aac5ede103", { v1: providerName });
    }
    if (providerName)
        return uiMessage("ui.d9b4e88fae4d", { v1: providerName });
    if (genres.includes(27))
        return "Uma escolha para apagar as luzes";
    if (genres.includes(35))
        return uiMessage("ui.58c35efb0fad");
    if (genres.includes(80) || genres.includes(53))
        return uiMessage("ui.65ab8aef6faf");
    if (genres.includes(878) || genres.includes(10765))
        return "Uma janela para outro universo";
    if (genres.includes(18))
        return "Drama com cara de descoberta editorial";
    return item.media_type === "tv" ? uiMessage("ui.dcc5fe32276a") : "Filme com energia de noite principal";
}
function providerTypeLabel(type?: string | null) {
    switch (type) {
        ...
        case "ads":
            return uiMessage("ui.8d40668af1ca");
        default:
            return "Disponibilidade";
    }
}
function FilterPill({ label, active, onClick, className = "", }: {
    label: string;
    active: boolean;
    onClick: () => void;
    className?: string;
}) {
    return (<button type="button" onClick={onClick} className={`min-h-10 min-w-0 shrink-0 whitespace-nowrap rounded-full border px-3 py-2 text-center text-[11px] font-bold transition sm:min-h-0 sm:px-3 sm:py-1.5 ${active
            ? "border-white/22 bg-white/12 text-white"
            : "border-white/10 bg-white/[0.03] text-white/42 hover:border-white/18 hover:text-white/70"} ${className}`}>
      {label}
    </button>);
}
function typeLabel(type: TypeFilter) {
    switch (type) {
        case "movie":
            return "Filmes";
        case "tv":
            return uiMessage("ui.de212174bc0c");
        default:
            return "Tudo";
    }
}
...
function mobileFilterSummary(poolMode: PoolMode, typeFilter: TypeFilter, vibeFilter: VibeFilter) {
...
function MysteryCard({ index, active, image, onClick, }: {
    index: number;
    active: boolean;
    image: string | null;
    onClick: () => void;
}) {
    return (<button type="button" onClick={onClick} className={`group relative h-[236px] overflow-hidden rounded-[22px] border text-left shadow-2xl transition duration-500 min-[390px]:h-[260px] sm:h-[420px] sm:rounded-[26px] ${active
            ? "scale-[1.02] border-cyan-200/35 shadow-cyan-950/40"
            : "border-white/10 shadow-black/40 hover:-translate-y-2 hover:border-white/24"}`} style={{ transformStyle: "preserve-3d" }}>
      ...
      <div className="relative flex h-full flex-col justify-between p-4 sm:p-6">
        ...
        <div>
          ...
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-white/38">{uiMessage("ui.99775c46fa0a")}</p>
          <p className="mt-2 text-sm font-semibold text-white/72">{uiMessage("ui.bfe08a2d3bb6")}</p>
        </div>
      </div>
    </button>);
}
function ProviderBadge({ item }: {
    ...
    if (!item.best_provider_name) {
        return (<div className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2 text-[12px] font-bold text-white/42">
        <Eye className="h-4 w-4"/>{uiMessage("ui.d9ce679d03dd")}</div>);
    }
    const providerName = getCanonicalProviderDisplayName({ name: item.best_provider_name }) ?? item.best_provider_name;
    ...
    return (<div className={`inline-flex min-w-0 items-center gap-3 rounded-2xl border px-3 py-2 backdrop-blur-md ${item.is_preferred_provider
            ? "border-cyan-200/28 bg-cyan-300/10"
            : "border-white/10 bg-white/[0.05]"}`}>
      {logo ? (<img src={logo} alt={providerName} className="h-8 w-8 rounded-lg object-contain"/>) : (<div className="h-8 w-8 rounded-lg bg-white/10"/>)}
...
function ResultCard({ item, saving, saved, dismissing, onSave, onAgain, onOpen, onDismiss, }: {
    item: AgendaItem;
    saving: boolean;
    saved: boolean;
    dismissing: boolean;
    ...
    return (<section className="relative isolate overflow-hidden rounded-[22px] border border-white/12 bg-[#070a12] shadow-2xl shadow-black/50 sm:rounded-[28px]">
      {backdrop && (<div className="absolute inset-0">
      ...
      <div className="relative grid min-w-0 gap-0 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div className="absolute left-4 top-4 z-10 aspect-[2/3] w-[96px] overflow-hidden rounded-[16px] border border-white/10 shadow-xl shadow-black/35 min-[390px]:w-[108px] lg:relative lg:left-auto lg:top-auto lg:h-full lg:min-h-[460px] lg:w-full lg:self-stretch lg:rounded-none lg:border-0 lg:shadow-none">
          {poster ? (<img src={poster} alt={item.title} className="h-full w-full object-cover"/>) : (<div className="grid h-full w-full place-items-center bg-white/[0.04]">
        ...
        <div className="flex min-h-0 min-w-0 flex-col justify-between p-4 sm:p-8 lg:min-h-[460px]">
          <div className="min-w-0">
            <div className="min-h-[144px] pl-[112px] min-[390px]:min-h-[162px] min-[390px]:pl-[128px] lg:min-h-0 lg:pl-0">
              <div className="mb-3 flex flex-wrap items-center gap-1.5 sm:mb-5 sm:gap-2">
                <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${isMovie
            ...
            : "border-cyan-300/24 bg-cyan-300/10 text-cyan-100/75"}`}>
                ...
                {genres.map((genre) => (<span key={genre} className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-bold text-white/38">
                    {genre}
                  </span>))}
              </div>
              ...
              <h2 className="max-w-2xl text-[25px] font-black leading-[1.04] text-white min-[390px]:text-[28px] sm:text-[46px] sm:leading-none lg:text-[52px]">
                {mainTitle}
                {subTitle && (<span className="mt-1 block text-[14px] font-light tracking-normal text-white/40 sm:text-[18px]">
                    {subTitle}
                  </span>)}
              </h2>
            </div>

            {item.overview && (<p className="mt-4 max-w-2xl text-[13px] leading-6 text-white/60 sm:mt-5 sm:text-sm">
                {item.overview}
              </p>)}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2.5 sm:mt-6 sm:gap-3">
              ...
              {(item.vote_average ?? 0) > 0 && (<div className="inline-flex items-center gap-2 rounded-2xl border border-amber-300/18 bg-amber-300/10 px-3 py-2 text-[12px] font-black text-amber-100/78">
          ...
          <div className="mt-7 sm:mt-8">
            <div className="mb-4 flex max-w-full items-start gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3.5 py-3 sm:mb-5 sm:inline-flex sm:items-center sm:px-4">
              ...
              <p className="min-w-0 text-[12px] font-semibold italic leading-5 text-white/56 sm:truncate">{uiMessage("ui.e2bbf209ae0e")}{contextMessage(item)}{uiMessage("ui.e2bbf209ae0e")}</p>
            </div>

            <div className="flex flex-wrap gap-2.5">
              <button type="button" onClick={onAgain} className="order-2 inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-[13px] font-black text-white/62 transition hover:border-white/18 hover:bg-white/[0.08] hover:text-white sm:order-none sm:flex-none sm:justify-start">
                <RotateCcw className="h-4 w-4"/>{uiMessage("ui.1245676ca2b0")}</button>
              <button type="button" onClick={onSave} disabled={saving || saved} className={`order-3 inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-[13px] font-black transition disabled:pointer-events-none sm:order-none sm:flex-none sm:justify-start ${saved
            ...
            : "border-cyan-300/24 bg-cyan-300/10 text-cyan-100 hover:bg-cyan-300/16"}`}>
              ...
              <button type="button" onClick={onOpen} className="order-1 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl border border-white/16 bg-white px-5 py-3 text-[13px] font-black text-[#070a12] transition hover:bg-cyan-50 sm:order-none sm:w-auto sm:justify-start">{uiMessage("ui.5e1b8c66a151")}<ArrowRight className="h-4 w-4"/>
              </button>
              <button type="button" onClick={onDismiss} disabled={dismissing} className="order-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-white/8 bg-transparent px-4 py-3 text-[13px] font-black text-white/28 transition hover:border-red-400/20 hover:bg-red-400/8 hover:text-red-300/70 disabled:pointer-events-none sm:order-none sm:w-auto sm:justify-start">
                <EyeOff className="h-4 w-4"/>{uiMessage("ui.7aea725c0eb9")}</button>
            </div>
          </div>
        </div>
      </div>
...
function LoadingSkeleton() {
    return (<div className="grid gap-3 sm:grid-cols-3">
      {Array.from({ length: 3 }).map((_, index) => (<div key={index} className="h-[236px] rounded-[22px] border border-white/8 bg-white/[0.035] min-[390px]:h-[260px] sm:h-[420px] sm:rounded-[26px]"/>))}
    </div>);
}
export default function SorteioPage() {
    ...
    const revealCard = useCallback(async (index: number) => {
        if (pool.length === 0)
            return;
        setActiveCard(index);
        setIsDrawing(true);
        try {
            ...
            window.setTimeout(() => {
                if (payload.data?.item)
                    setRevealedItem(payload.data.item);
                if (payload.data?.meta)
                    setPoolMeta(payload.data.meta);
                setIsDrawing(false);
            }, 260);
        }
        catch (err) {
    ...
    return (<div className="relative -mx-4 min-h-[calc(100dvh-80px)] overflow-hidden px-4 pb-24 sm:-mx-6 sm:px-6 md:-mx-8 md:px-8 md:pb-20 lg:-mx-10 lg:px-10">
      ...
      <div className="mx-auto max-w-[1120px] pt-5 sm:pt-10">
        <header className="mb-5 flex flex-col gap-4 sm:mb-7 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-3 flex items-center gap-2">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.26em] text-cyan-100/62">{uiMessage("ui.0afa4ffdaee4")}</p>
            </div>
            <h1 className="text-[42px] font-black leading-none tracking-[-0.04em] text-white sm:text-[72px] sm:tracking-[-0.055em]">{uiMessage("ui.45a25136ea07")}</h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-white/42">
              {displayedPoolCount}{uiMessage("ui.351f539a27c7")}{fallbackActive ? " · fallback geral ativo" : ""}
          ...
          <div className="hidden gap-2 sm:flex sm:flex-wrap sm:items-center lg:justify-end">
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
              ...
              <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.7a4304790175")} active={poolMode === "watchlist"} onClick={() => setSourceMode("watchlist")}/>
            </div>
            <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
              ...
              <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.de212174bc0c")} active={typeFilter === "tv"} onClick={() => setTypeMode("tv")}/>
            </div>
          </div>
        </header>

        <div className="mb-5 sm:hidden">
          <button type="button" onClick={() => setMobileFiltersOpen((open) => !open)} className="flex min-h-12 w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] px-3.5 py-2.5 text-left transition hover:border-white/18 hover:bg-white/[0.07]" aria-expanded={mobileFiltersOpen}>
          ...
          {mobileFiltersOpen && (<div className="mt-2 rounded-2xl border border-white/10 bg-black/18 p-2.5">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                ...
                <FilterPill className="w-full" label={uiMessage("ui.de212174bc0c")} active={typeFilter === "tv"} onClick={() => setTypeMode("tv")}/>
              </div>
              ...
              <button type="button" onClick={resetReveal} className="mt-2 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-2 text-[11px] font-bold text-white/42 transition hover:text-white/70">
        ...
        <div className="mb-6 hidden grid-cols-2 gap-2 sm:mb-8 sm:flex sm:flex-wrap">
          <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.0b699238f6ff")} active={vibeFilter === "all"} onClick={() => setQuickMode("all")}/>
          <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.9b97e8e89d6a")} active={vibeFilter === "surprise"} onClick={() => setQuickMode("surprise")}/>
          <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.59f25599e7cf")} active={vibeFilter === "intense"} onClick={() => setQuickMode("intense")}/>
          <FilterPill className="w-full sm:w-auto" label={uiMessage("ui.fbd1ced29e56")} active={vibeFilter === "light"} onClick={() => setQuickMode("light")}/>
          <button type="button" onClick={resetReveal} className="col-span-2 inline-flex min-h-10 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full border border-white/10 bg-white/[0.03] px-3 py-2 text-[11px] font-bold text-white/42 transition hover:text-white/70 sm:col-span-1 sm:min-h-0 sm:w-auto sm:px-3 sm:py-1.5">
            <Shuffle className="h-3.5 w-3.5"/>
            Reembaralhar
          </button>
        </div>

        {isLoading ? (<LoadingSkeleton />) : pool.length === 0 ? (<div className="grid min-h-[360px] place-items-center rounded-[28px] border border-white/10 bg-white/[0.035] text-center">
            <p className="max-w-sm px-6 text-sm font-semibold text-white/48">
              {poolMode === "watchlist"
                ? uiMessage("ui.acbb2a66e7c7") : uiMessage("ui.f8f1a38a3a05")}
            </p>
          </div>) : revealedItem ? (<ResultCard item={revealedItem} saved={savedKeys.has(itemKey(revealedItem))} saving={savingKey === itemKey(revealedItem)} dismissing={isDismissing} onSave={saveCurrent} onAgain={resetReveal} onOpen={() => router.push(`/title/${revealedItem.media_type}/${revealedItem.id}`)} onDismiss={dismissCurrent}/>) : (<div className="grid gap-3 sm:grid-cols-3">
            {[0, 1, 2].map((index) => (<MysteryCard key={index} index={index} active={activeCard === index} image={previewImages[index]} onClick={() => {
                    if (!isDrawing)
