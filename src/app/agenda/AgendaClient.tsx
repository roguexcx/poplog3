...
// ── Constantes ─────────────────────────────────────────────────────────────────
const TMDB_IMG = (path: string | null | undefined, size: string) => resolveCatalogImage(path, size);
...
function bestHorizontalImg(tmdb: {
    backdrop_path: string | null;
    poster_path: string | null;
    clean_backdrop_path?: string | null;
}, size = "w780"): string | null {
...
const PHASE_LABELS: Record<Phase, string> = {
    ...
    grouping: uiMessage("ui.eb98cc59921c"),
    cache_check: "Consultando cache…",
    enriching: "Enriquecendo dados…",
    done: uiMessage("ui.8320aee0a8bc"),
};
// ── Tipos de view ──────────────────────────────────────────────────────────────
...
function isPremiereEpisode(group: IcsSeriesGroup, dateStr: string): boolean {
    ...
    if (group.key.startsWith("tmdb-") && group.tmdb?.first_air_date) {
        ...
        if (nowMs - airMs > 180 * 24 * 3600 * 1000)
            return false;
    }
    return true;
}
...
function brazilBonus(group: IcsSeriesGroup): number {
    ...
    if (cat === "REALITY" || cat === "PODCAST" || cat === "DAILY_SOAP")
        return 0;
...
function groupEditorialScore(group: IcsSeriesGroup, dateStr: string, trendingDay: Set<number>, trendingWeek: Set<number>): number {
    ...
    return ((group.relevanceScore ?? 0) +
        ...
        ((() => {
            ...
            if (reduced.has(lang))
                return dayBoost ? 19 : weekBoost ? 10 : 0;
            if (lang === "ja" && group.category !== "ANIMATION")
                return dayBoost ? 10 : weekBoost ? 5 : 0;
...
function editorialSignal(group: IcsSeriesGroup, dateStr: string, trendingDay: Set<number>, trendingWeek: Set<number>, movie?: MovieGroup) {
    // Sinais para filmes
    if (movie) {
        const tmdbId = movie.movie.tmdb_id;
        if (trendingDay.has(tmdbId))
            return { label: uiMessage("ui.527832d33724"), color: "rose" as const };
        const releaseDate = movie.movie.release_date ?? dateStr;
        const days = daysUntilDate(releaseDate);
        if (days === 0)
            return { label: uiMessage("ui.a711ee61cb33"), color: "emerald" as const };
        if (days > 0 && days <= 3)
            return { label: uiMessage("ui.0f6dfca7d303"), color: "cyan" as const };
        if (days > 3 && days <= 14)
            return { label: uiMessage("ui.0db301482fbd", { v1: days }), color: "cyan" as const };
        if (days > 14)
            return { label: uiMessage("ui.efb4c97d4d2c"), color: "slate" as const };
        ...
        if ((movie.relevanceScore ?? 0) >= 65)
            return { label: uiMessage("ui.7aca214a611c"), color: "sky" as const };
        return { label: uiMessage("ui.35ba0d43cb40"), color: "slate" as const };
    }
    // Sinais para séries
    const tmdbId = group.tmdb?.tmdb_id;
    ...
    if (isSeasonFinaleGuess(group, dateStr))
        return { label: uiMessage("ui.ad0ef0fce87e"), color: "violet" as const };
    // "Estreia de série" só para S01E01 — série completamente nova
    if (isPremiereEpisode(group, dateStr))
        return { label: uiMessage("ui.8ec4505e01de"), color: "emerald" as const };
    // Nova temporada: E01 de qualquer season > 1
    if (isSeasonStart(group, dateStr))
        return { label: uiMessage("ui.b5fa08920aa0"), color: "cyan" as const };
    if (trendingWeek.has(tmdbId ?? -1))
        return { label: "Trending", color: "amber" as const };
    if ((group.relevanceScore ?? 0) >= 70)
        return { label: uiMessage("ui.5209e8d95203"), color: "sky" as const };
    return { label: uiMessage("ui.829d6c18f768"), color: "slate" as const };
}
type EditorialGroup = {
...
function movieEditorialScore(movie: MovieGroup, trendingDay: Set<number>, trendingWeek: Set<number>): number {
    const m = movie.movie;
    ...
    const rawScore = (movie.relevanceScore ?? 0) +
...
function buildEditorialGroups(sourceGroups: IcsSeriesGroup[], sourceMovies: MovieGroup[], options: {
    mode: ViewMode;
    selectedDay: string;
    weekStart: Date;
    year: number;
    ...
    const rangeStart = options.mode === "day" ? options.selectedDay :
        options.mode === "week" ? toLocalDateStr(options.weekStart) :
            /* month */ windowStart;
    const rangeEnd = (() => {
        if (options.mode === "day")
            return options.selectedDay;
    ...
    for (const movie of sourceMovies) {
        ...
        if (releaseDate < rangeStart || releaseDate > rangeEnd)
            continue;
        const key = movie.key;
        if (seen.has(key))
            continue;
    ...
    return items.map((item) => {
        ...
        if (!shouldPromote)
            return { ...item, visualWeight: "compact" as const };
        if (needsLanguageCap)
            promotedLangs.add(lang);
        promoted++;
...
const CAT_LABEL: Partial<Record<ContentCategory, string>> = {
    ...
    SERIES: uiMessage("ui.74f19285073a"), ANIMATION: uiMessage("ui.fad2415d4676"),
...
function isCinematicPrestige(group: IcsSeriesGroup): boolean {
    ...
    return (networks.some((n) => prestigeNetworkIds.has(n.id)) ||
...
function cinematicFallbackLabel(group: IcsSeriesGroup): string {
    ...
    if (genres.some((g) => /crime|mistério|thriller/i.test(g)))
        return uiMessage("ui.d91779a7e8a0");
    if (genres.some((g) => /drama/i.test(g)))
        return "Drama";
    if (genres.some((g) => /ficção|sci.fi|fantasy/i.test(g)))
        return uiMessage("ui.67635af7aa71");
    return "Cinematic";
}
...
function resolveCatLabel(group: IcsSeriesGroup): {
    ...
    if (group.category === "CINEMATIC")
        return resolveCinematicLabel(group);
...
function buildSpotlightItems(featuredGroups: IcsSeriesGroup[], trendingDay: Set<number>, trendingWeek: Set<number>): SpotlightItem[] {
    ...
    for (const g of featuredGroups) {
        ...
        const upcomingEp = g.episodes.find((ep) => ep.startAt.slice(0, 10) >= today && ep.startAt.slice(0, 10) <= nextWeekStr);
        if (!upcomingEp)
            continue;
        const dateStr = upcomingEp.startAt.slice(0, 10);
        const daysUntil = Math.ceil((new Date(dateStr + "T12:00:00").getTime() - Date.now()) / 86400000);
        const label = daysUntil <= 0 ? "Hoje" :
            daysUntil === 1 ? uiMessage("ui.60cf8de1ba68") :
    ...
    items.sort((a, b) => {
        const scoreA = (a.isTrendingDay ? 100 : 0) +
            (a.isPremiere ? 50 : 0) +
            (a.isFinale ? 40 : 0) +
            (a.isTrendingWeek ? 30 : 0) +
            (a.group.relevanceScore ?? 0);
        const scoreB = (b.isTrendingDay ? 100 : 0) +
            (b.isPremiere ? 50 : 0) +
            (b.isFinale ? 40 : 0) +
...
function SignalBadge({ label, color, }: {
    ...
    return (<span className={`inline-flex items-center rounded-lg border px-2 py-1 text-[9px] font-black uppercase ${SIGNAL_STYLES[color]}`}>
      {label}
    </span>);
}
...
function resolveItemData(item: EditorialGroup): NormalizedItem {
    ...
    if (movie) {
        ...
        const dateLabel = days < -7 ? "Nos cinemas" : days < 0 ? uiMessage("ui.983aae8da091", { v1: Math.abs(days) }) : days === 0 ? "Estreia hoje" : days === 1 ? uiMessage("ui.60cf8de1ba68") : `Em ${days} dias`;
    ...
    const tmdb = group.tmdb!;
    const days = daysUntilDate(dateStr);
    const dateLabel = days <= 0 ? "Hoje" : days === 1 ? uiMessage("ui.60cf8de1ba68") : days <= 7 ? `Em ${days} dias` : `Em ${days} dias`;
...
function AgendaEditorialHeroCard({ item, trendingDay, trendingWeek, }: {
    item: EditorialGroup;
    trendingDay: Set<number>;
    trendingWeek: Set<number>;
}) {
    ...
    return (<a href={d.href} className="group relative w-full h-full overflow-hidden rounded-[26px] border border-white/[0.08] bg-zinc-950/80 text-left shadow-[0_18px_44px_rgba(0,0,0,0.34)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-white/[0.16] block">
      ...
      <div className="relative flex h-full flex-col justify-between p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {!item.movie && item.group.streamingProvider && (<span className="rounded-lg border border-emerald-500/20 bg-emerald-500/15 px-2 py-1 text-[9px] font-black uppercase text-emerald-300/80">
          ...
          {d.voteAvg > 0 && (<span className="rounded-lg border border-amber-400/20 bg-amber-500/10 px-2 py-1 text-[11px] font-black text-amber-200">
              ★ {d.voteAvg.toFixed(1)}
            </span>)}
        </div>

        <div className="flex items-end gap-5">
          {d.poster && (<div className="hidden w-[104px] overflow-hidden rounded-2xl border border-white/[0.10] bg-white/[0.04] shadow-[0_14px_32px_rgba(0,0,0,0.30)] sm:block">
...
function AgendaEditorialWideCard({ item, trendingDay, trendingWeek, }: {
    item: EditorialGroup;
    trendingDay: Set<number>;
    trendingWeek: Set<number>;
}) {
    const d = resolveItemData(item);
    const signal = editorialSignal(item.group, item.dateStr, trendingDay, trendingWeek, item.movie);
    return (<a href={d.href} className="group relative w-full h-full overflow-hidden rounded-[24px] border border-white/[0.08] bg-zinc-950/75 text-left shadow-[0_14px_34px_rgba(0,0,0,0.30)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-white/[0.15] block">
      ...
      <div className="relative flex h-full flex-col justify-between p-5">
        ...
        <div className="max-w-[560px]">
          ...
          {d.overview && (<p className="mt-2 line-clamp-2 text-[12px] leading-relaxed text-white/52">
              {d.overview}
            </p>)}
        </div>
      </div>
...
function AgendaEditorialPosterCard({ item, trendingDay, trendingWeek, }: {
    item: EditorialGroup;
    trendingDay: Set<number>;
    trendingWeek: Set<number>;
}) {
    ...
    return (<a href={d.href} className="group relative w-full h-full overflow-hidden rounded-[24px] border border-white/[0.08] bg-zinc-950/75 text-left shadow-[0_14px_34px_rgba(0,0,0,0.30)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-white/[0.15] block">
      ...
      <div className="relative flex h-full flex-col justify-between p-4">
        <div className="flex items-start justify-between gap-2">
          <SignalBadge label={signal.label} color={signal.color}/>
          {d.voteAvg > 0 && (<span className="rounded-lg border border-amber-400/15 bg-black/20 px-1.5 py-1 text-[10px] font-black text-amber-200/90">
              ★ {d.voteAvg.toFixed(1)}
            </span>)}
        </div>
        <div>
          ...
          {d.overview && (<p className="mt-2 line-clamp-2 text-[11px] leading-relaxed text-white/50">
              {d.overview}
            </p>)}
        </div>
      </div>
...
function AgendaEditorialSquareCard({ item, trendingDay, trendingWeek, }: {
    item: EditorialGroup;
    trendingDay: Set<number>;
    trendingWeek: Set<number>;
}) {
    ...
    const signal = editorialSignal(item.group, item.dateStr, trendingDay, trendingWeek, item.movie);
    return (<a href={d.href} className="group relative w-full h-full overflow-hidden rounded-[24px] border border-white/[0.08] bg-zinc-950/75 text-left shadow-[0_14px_34px_rgba(0,0,0,0.28)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-white/[0.15] block">
      ...
      <div className="relative flex h-full flex-col justify-between p-4">
        <div className="flex items-center justify-between gap-2">
          <SignalBadge label={signal.label} color={signal.color}/>
          {d.voteAvg > 0 && (<span className="rounded-lg border border-amber-400/15 bg-black/25 px-1.5 py-1 text-[10px] font-black text-amber-200/90">
              ★ {d.voteAvg.toFixed(1)}
            </span>)}
        </div>
        <div>
...
function AgendaEditorialTallCard({ item, trendingDay, trendingWeek, }: {
    item: EditorialGroup;
    trendingDay: Set<number>;
    trendingWeek: Set<number>;
}) {
    ...
    const signal = editorialSignal(item.group, item.dateStr, trendingDay, trendingWeek, item.movie);
    return (<a href={d.href} className="group relative w-full h-full overflow-hidden rounded-[24px] border border-white/[0.08] bg-zinc-950/75 text-left shadow-[0_14px_34px_rgba(0,0,0,0.28)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-white/[0.16] block">
      {poster ? (<img src={poster} alt="" className="absolute inset-0 h-full w-full object-cover opacity-90 transition-transform duration-700 group-hover:scale-[1.04]"/>) : (<div className="absolute inset-0 bg-gradient-to-b from-zinc-800 to-zinc-950"/>)}
      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent"/>
      <div className="relative flex h-full flex-col justify-between p-3.5">
        <div className="flex items-center justify-between gap-2">
          <SignalBadge label={signal.label} color={signal.color}/>
          {d.voteAvg > 0 && (<span className="rounded-lg border border-amber-400/15 bg-black/30 px-1.5 py-1 text-[10px] font-black text-amber-200/90">
              ★ {d.voteAvg.toFixed(1)}
            </span>)}
        </div>
        <div>
...
function AgendaCompactCluster({ items }: {
    items: EditorialGroup[];
}) {
    if (items.length === 0)
        return null;
    return (<div className="col-span-1 rounded-[24px] border border-white/[0.08] bg-zinc-900/80 shadow-[0_18px_50px_rgba(0,0,0,0.40)] backdrop-blur-xl p-4 sm:col-span-2 lg:col-span-4">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          <p className="text-[9px] font-black uppercase text-white/30">{uiMessage("ui.0f2df427042e")}</p>
          <h3 className="text-[18px] font-black text-white">{uiMessage("ui.54f28cc45101")}</h3>
        </div>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {items.slice(0, 12).map((editorialItem) => {
            ...
            return (<a key={`${editorialItem.movie ? `movie-${d.tmdbId}` : editorialItem.group.key}-${editorialItem.dateStr}`} href={d.href} className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.035] p-2.5 text-left transition-colors hover:border-white/[0.10] hover:bg-white/[0.05]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[10px] font-bold text-white/35">
                  {d.dateLabel}{d.subLabel ? ` · ${d.subLabel}` : ""}
                  {!editorialItem.movie && editorialItem.group.streamingProvider && (<span className="ml-1.5 text-emerald-400/70">· {editorialItem.group.streamingProvider.name}</span>)}
                </p>
              </div>
            </a>);
        })}
...
function assignSlots(items: EditorialGroup[], mode: ViewMode): Array<{
    item: EditorialGroup;
    slot: SlotDef;
    rowH: string;
}> {
...
function AgendaEditorialFeed({ items, mode, isLoading, trendingDay, trendingWeek, }: {
    items: EditorialGroup[];
    mode: ViewMode;
    isLoading: boolean;
    trendingDay: Set<number>;
    ...
    if (isLoading && items.length === 0) {
        ...
        {rows.slice(0, 2).map((row, rIdx) => {
                ...
                return (<div key={rIdx} className={`grid gap-3 ${rowH}`} style={{ gridTemplateColumns: "repeat(12, 1fr)" }}>
    ...
    if (nonCmp.length === 0) {
        return (<div className="rounded-[24px] border border-white/[0.08] bg-zinc-900/80 px-6 py-14 text-center">
        <p className="text-[14px] font-black text-white/35">{uiMessage("ui.eb490825a4f7")}</p>
        <p className="mt-1 text-[11px] text-white/20">{uiMessage("ui.eb2cb20e7a5e")}</p>
      </div>);
    }
    ...
    return (<div className="flex flex-col gap-3">
      {renderedRows.map((row, rIdx) => {
            ...
            {row.items.map((item, cIdx) => {
                    ...
                    return (<div key={key} className="h-full min-w-0" style={{ gridColumn: `span ${slot.colSm}` }}>
...
function RadarHero({ phase, mode, onChangeMode, filteredCount, filteredEps, enrichProgress, spotlightItems, }: {
    phase: Phase;
    mode: ViewMode;
    ...
    const badge = item
        ? (item.isTrendingDay ? { text: uiMessage("ui.83760022f740"), cls: "bg-rose-500/20 text-rose-300 border-rose-500/25" }
            : item.isPremiere ? { text: "Estreia", cls: "bg-emerald-500/20 text-emerald-300 border-emerald-500/25" }
                : item.isFinale ? { text: uiMessage("ui.ad0ef0fce87e"), cls: "bg-violet-500/20 text-violet-300 border-violet-500/25" }
                    : item.isTrendingWeek ? { text: uiMessage("ui.9ad3a3e65198"), cls: "bg-amber-500/20 text-amber-300 border-amber-500/25" }
                        : null)
        : null;
    const href = tmdb ? `/title/tv/${tmdb.tmdb_id}` : "#";
    return (<div className="relative isolate mb-9 overflow-hidden rounded-[2rem] border border-white/[0.10] shadow-[0_32px_100px_rgba(0,0,0,0.55)]">
      {/* Fundo base */}
      ...
      <div className="relative px-6 pt-6 pb-0 sm:px-9 sm:pt-8">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="text-[10px] font-black uppercase tracking-[0.18em] text-sky-400/80">{uiMessage("ui.0f596df65aca")}</span>
              <span className={`flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border ${phase === "done"
          ...
          <div className="flex items-center gap-1 rounded-2xl border border-white/[0.09] bg-black/30 p-1 backdrop-blur-md shrink-0">
            {(["day", "week", "month"] as ViewMode[]).map((v) => (<button key={v} type="button" onClick={() => onChangeMode(v)} className={`text-[11px] font-bold px-3.5 py-1.5 rounded-lg transition-all duration-200 ${mode === v
        ...
        <div className="flex items-center gap-4 flex-wrap mt-4 mb-6">
          <h1 className="text-4xl sm:text-5xl font-black text-white leading-none tracking-[-0.06em]">{uiMessage("ui.aad82db2cb77")}</h1>
          {filteredCount > 0 && (<>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] text-white/30">{uiMessage("ui.4c07aff1feae")}</span>
              </div>
              <div className="h-4 w-px bg-white/10"/>
              <div className="flex items-center gap-1.5">
                <span className="text-2xl font-black text-emerald-300 tabular-nums">{filteredEps.toLocaleString("pt-BR")}</span>
                <span className="text-[11px] text-white/30">{uiMessage("ui.f869eb17cc2f")}</span>
              </div>
              {phase === "enriching" && (<>
                  <div className="h-4 w-px bg-white/10"/>
                  <span className="text-[11px] text-white/30">{uiMessage("ui.37237a7a2574")}{Math.round(enrichProgress)}%</span>
                </>)}
            </>)}
          {filteredCount === 0 && phase !== "done" && (<span className="text-[13px] text-white/35 ml-2">{PHASE_LABELS[phase]}</span>)}
        </div>
      </div>

      {/* ── Hero cinematográfico — continua o mesmo fundo, sem borda separada ── */}
      {spotlightItems.length > 0 && item && tmdb && (<div className="relative cursor-pointer overflow-hidden" style={{ minHeight: 220 }} onClick={() => router.push(href)}>
          ...
          {spotlightItems.length > 1 && (<>
              <button type="button" aria-label="Anterior" onClick={(e) => {
                    e.stopPropagation();
                    if (timerRef.current)
                        clearTimeout(timerRef.current);
                    goTo((idx - 1 + spotlightItems.length) % spotlightItems.length);
              ...
              <button type="button" aria-label={uiMessage("ui.64b2d0d63475")} onClick={(e) => {
                    e.stopPropagation();
                    if (timerRef.current)
                        clearTimeout(timerRef.current);
                    goTo((idx + 1) % spotlightItems.length);
          ...
          <div className="relative flex items-end gap-5 p-5 sm:p-7 min-h-[240px] transition-opacity duration-300" style={{ opacity: visible ? 1 : 0 }}>
            {poster && (<div className="hidden sm:block w-[80px] shrink-0 rounded-xl overflow-hidden border border-white/[0.10] shadow-xl shadow-black/40">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
                ...
                {item.group.streamingProvider && (<span className="text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-lg border bg-emerald-500/15 text-emerald-300/80 border-emerald-500/20">
                    {item.group.streamingProvider.name}
                  </span>)}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {(tmdb.vote_average ?? 0) > 0 && (<div className="flex items-center gap-1">
            ...
            {/* Setas + dots desktop */}
            {spotlightItems.length > 1 && (<div className="hidden sm:flex flex-col items-center gap-2 shrink-0 self-center">
                <button type="button" aria-label="Anterior" onClick={(e) => {
                    e.stopPropagation();
                    if (timerRef.current)
                        clearTimeout(timerRef.current);
                ...
                <div className="flex flex-col gap-1 mt-1">
                  {spotlightItems.slice(0, Math.min(spotlightItems.length, 8)).map((_, i) => (<button key={i} type="button" onClick={(e) => {
                        e.stopPropagation();
                        if (timerRef.current)
                            clearTimeout(timerRef.current);
                        goTo(i);
                    }} className={`rounded-full transition-all duration-300 ${i === idx ? "h-4 w-1.5 bg-white/60" : "h-1.5 w-1.5 bg-white/20 hover:bg-white/35"}`} aria-label={uiMessage("ui.acbfb9e1bf50", { v1: i + 1 })}/>))}
                </div>
              </div>)}
          </div>
...
const WEEK_HEADERS = ["Seg", "Ter", "Qua", "Qui", "Sex", uiMessage("ui.b5310f24fcbb"), "Dom"];
function MonthView({ year, month, byDay, selectedDay, onSelectDay, }: {
    year: number;
    month: number;
    ...
    return (<div>
      <div className="grid grid-cols-7 mb-1.5">
        {WEEK_HEADERS.map((h) => (<div key={h} className="text-center text-[11px] font-bold uppercase tracking-[0.14em] text-white/30 py-2.5">{h}</div>))}
      </div>
      <div className="grid grid-cols-7 gap-1">
        {cells.map(({ dateStr, isCurrent }) => {
            ...
            const pillColor = (g: IcsSeriesGroup) => {
                const c = g.category;
                if (c === "CINEMATIC")
                    return "bg-violet-500/20 border-violet-500/20 text-violet-200/80";
                if (c === "ANIMATION")
                    return "bg-teal-500/20 border-teal-500/20 text-teal-200/80";
                if (c === "DOCUMENTARY")
                    return "bg-cyan-500/20 border-cyan-500/20 text-cyan-200/80";
                if (c === "REALITY_PREMIUM")
                    return "bg-amber-500/20 border-amber-500/20 text-amber-200/80";
                return "bg-sky-500/15 border-sky-500/15 text-sky-200/75";
            };
            return (<button key={dateStr} type="button" onClick={() => onSelectDay(dateStr)} className={[
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {hasContent && (<span className={`text-[9px] font-black tabular-nums ${isToday ? "text-sky-400/80" : "text-white/20"}`}>
                    {groups.length}
                  </span>)}
              </div>
              <div className="flex flex-col gap-[3px]">
                {visible.map((g) => {
                    ...
                    return (<div key={g.key} className={`flex items-center gap-1 rounded-[5px] px-1.5 py-[3px] border ${pc}`}>
                      {poster && (<img src={poster} alt="" className="w-3.5 h-5 rounded-[2px] object-cover shrink-0 opacity-90"/>)}
                      <span className="text-[9px] font-bold truncate leading-tight">
                        {g.tmdb?.name ?? g.rawTitle}
                      </span>
                    </div>);
                })}
                {extra > 0 && (<span className="text-[9px] font-bold text-white/30 pl-1">+{extra} mais</span>)}
              </div>
            </button>);
        })}
      </div>
...
// ── Vista Semanal ──────────────────────────────────────────────────────────────
function WeekView({ weekStart, byDay, selectedDay, onSelectDay, }: {
    weekStart: Date;
    byDay: Map<string, IcsSeriesGroup[]>;
    selectedDay: string;
    onSelectDay: (d: string) => void;
    ...
    const DAY_SHORT = ["Seg", "Ter", "Qua", "Qui", "Sex", uiMessage("ui.b5310f24fcbb"), "Dom"];
    return (<div className="space-y-2">
      {days.map((dateStr, i) => {
            ...
            {groups.length === 0 ? (<span className="text-[11px] text-white/15 flex-1 italic">{uiMessage("ui.fbcca1f960bc")}</span>) : (<div className="flex-1 flex flex-wrap gap-1.5 items-center">
                {groups.slice(0, 6).map((g) => {
                        const poster = g.tmdb ? TMDB_IMG(g.tmdb.poster_path, "w92") : null;
                        const c = g.category;
                        const pc = c === "CINEMATIC" ? "bg-violet-500/20 border-violet-500/20 text-violet-200/85" :
                        ...
                        return (<div key={g.key} className={`flex items-center gap-1.5 rounded-lg px-2 py-1 border ${pc}`}>
                      {poster && (<img src={poster} alt="" className="w-4 h-6 rounded-[3px] object-cover shrink-0 opacity-90"/>)}
                      <span className="text-[10px] font-bold max-w-[100px] truncate leading-tight">
                        {g.tmdb?.name ?? g.rawTitle}
                      </span>
                    </div>);
                    })}
                {groups.length > 6 && (<span className="text-[10px] text-white/30 font-bold">+{groups.length - 6}</span>)}
              </div>)}

            {groups.length > 0 && (<span className="shrink-0 text-[13px] font-black text-white/20 tabular-nums">{groups.length}</span>)}
          </button>);
        })}
    </div>);
}
// ── Vista Diária ───────────────────────────────────────────────────────────────
function DayView({ dateStr, groups, }: {
    dateStr: string;
    ...
    if (groups.length === 0) {
        return (<div className="rounded-2xl border border-white/[0.05] bg-white/[0.015] px-6 py-12 text-center">
        <p className="text-[13px] font-bold text-white/25 mb-1">{uiMessage("ui.0d498de6bbd1")}</p>
        <p className="text-[11px] text-white/15">{label}</p>
      </div>);
    }
    // Ordena grupos por relevância decrescente (hype + popularidade + tendência)
    const sortedGroups = [...groups].sort((a, b) => (b.relevanceScore ?? 0) - (a.relevanceScore ?? 0) ||
        ((b.tmdb?.popularity ?? 0) - (a.tmdb?.popularity ?? 0)));
    return (<div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <SectionEyebrow color="sky">{uiMessage("ui.276aaacdf374")}</SectionEyebrow>
        ...
        <span className="text-[11px] font-black text-white/20 border border-white/[0.08] rounded-full px-2.5 py-0.5">
          {sortedGroups.length}{uiMessage("ui.f6932c188eba")}{sortedGroups.length !== 1 ? "s" : ""}
        </span>
      </div>
      <div className="space-y-2">
        {sortedGroups.map((g) => {
            ...
            return (<div key={g.key} className="group relative rounded-[22px] border border-white/[0.08] bg-white/[0.035] backdrop-blur-xl hover:border-white/[0.10] transition-all duration-300 overflow-hidden cursor-pointer" onClick={() => g.tmdb?.tmdb_id && router.push(`/title/tv/${g.tmdb.tmdb_id}`)}>
              {g.tmdb?.backdrop_path && (<div className="absolute inset-0 opacity-[0.12]">
              ...
              <div className="relative flex items-start gap-4 p-4">
                <div className="relative w-[56px] h-[84px] rounded-2xl overflow-hidden bg-white/[0.04] shrink-0 border border-white/[0.08]">
                  {g.tmdb?.poster_path ? (<img src={TMDB_IMG(g.tmdb.poster_path, "w185")!} alt={g.tmdb.name} className="h-full w-full object-cover" loading="lazy"/>) : (<div className="h-full w-full flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {showTime && (<span className="text-[10px] font-bold text-white/30 border border-white/[0.07] rounded-full px-2 py-0.5">
                        {formatTime(firstEpTime)}
                      </span>)}
                    {g.tmdb?.vote_average && g.tmdb.vote_average > 0 && (<span className="flex items-center gap-1 text-[10px] font-bold text-amber-300/80">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {eps.map((ep) => (<span key={ep.uid} className="text-[10px] font-black px-2 py-0.5 rounded-full bg-white/[0.07] text-white/55 border border-white/[0.10]">
                ...
                {eps.length > 1 && (<div className="shrink-0 text-right">
...
function PeriodNav({ label, onPrev, onNext, onToday }: {
    ...
    return (<div className="flex items-center justify-between mb-5">
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={onToday} className="text-[11px] font-bold text-white/40 hover:text-white/65 border border-white/[0.09] rounded-2xl px-3.5 py-1.5 transition-colors">
          Hoje
        </button>
        {(["prev", "next"] as const).map((dir) => (<button key={dir} type="button" onClick={dir === "prev" ? onPrev : onNext} aria-label={dir === "prev" ? "Anterior" : uiMessage("ui.64b2d0d63475")} className="w-9 h-9 rounded-2xl border border-white/[0.09] bg-white/[0.045] flex items-center justify-center text-white/40 hover:text-white/75 hover:bg-white/[0.07] transition-all">
...
function DayPanel({ dateStr, groups, onClose }: {
    ...
    return (<div className="mt-8 rounded-[28px] border border-sky-500/20 bg-sky-950/10 p-5 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        <SectionEyebrow color="sky">{uiMessage("ui.417257875f42")}</SectionEyebrow>
        <button type="button" onClick={onClose} className="text-[10px] font-bold text-white/25 hover:text-white/50 border border-white/[0.07] rounded-lg px-2.5 py-1 transition-colors">
          Fechar
        </button>
      </div>
...
// ── Page ───────────────────────────────────────────────────────────────────────
export default function AgendaClient({ initialData }: {
    ...
    const hydrate = (gs: IcsSeriesGroup[]): IcsSeriesGroup[] => gs.map((g) => ({ ...g, episodes: g.episodes.map((ep) => ({ ...ep })) }));
    const [groups, setGroups] = useState<IcsSeriesGroup[]>(() => initialData ? hydrate(initialData.groups ?? []) : []);
    const [featuredGroups, setFeatured] = useState<IcsSeriesGroup[]>(() => initialData ? hydrate(initialData.featuredGroups ?? []) : []);
                    ...
                    try {
                        ...
                        if (enrichRes.ok) {
                            const { results } = await enrichRes.json() as {
                                results: Record<string, import("@/lib/ics-engine").TmdbEnrichment | null>;
                            };
                            const apply = (prev: IcsSeriesGroup[]) => prev
                                ...
                                if (ALL_BLOCKED_CATEGORIES.has(cat))
                                    return false;
                                g.isRelevant = true;
                                return true;
                            });
                        ...
                        console.warn("[agenda] enrich batch error:", e);
                    }
                    done += batch.length;
    ...
    const filteredEps = useMemo(() => visibleFeatured.reduce((acc, g) => acc + g.episodeCount, 0), [visibleFeatured]);
    ...
    const periodLabel = useMemo(() => {
        if (mode === "month")
            return uiMessage("ui.a01e83c7c231");
        if (mode === "week")
            return `Semana de ${formatWeekRange(navWeekStart)}`;
        return formatDayFull(selectedDay);
    }, [mode, navYear, navMonth, navWeekStart, selectedDay]);
    const spotlightItems = useMemo(() => buildSpotlightItems(visibleFeatured, trendingDayRef.current, trendingWeekRef.current), [visibleFeatured]);
    const isLoading = phase !== "done" && phase !== "idle";
    const editorialItems = useMemo(() => buildEditorialGroups(visibleFeatured, movies, {
        mode,
        selectedDay,
    ...
    return (<PageShell variant="wide">
      ...
      <RadarHero phase={phase} mode={mode} onChangeMode={setMode} filteredCount={filteredCount} filteredEps={filteredEps} enrichProgress={enrichProgress} spotlightItems={spotlightItems}/>

      {error && (<div className="mb-6 rounded-2xl border border-red-500/20 bg-red-950/20 px-5 py-4 text-[12px] text-red-300/80">{uiMessage("ui.c21f42f3ce8a")}{error}
        </div>)}
      ...
      <section>
        <AgendaEditorialFeed items={editorialItems} mode={mode} isLoading={isLoading} trendingDay={trendingDayRef.current} trendingWeek={trendingWeekRef.current}/>
      </section>
      ...
      <div className="mt-10 flex items-center gap-2 border-t border-white/[0.05] pt-6">
        ...
        <ContextualAttribution context="calendar" sourcesUsed={["bancodeseries", "tmdb"]}/>
      </div>
