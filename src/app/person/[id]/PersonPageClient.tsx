...
import type { PoplogPersonCredit, PoplogPersonPageData, } from "@/server/poplog-people/getPersonPageData";
// ── Types ─────────────────────────────────────────────────────────────────────
...
const DEPARTMENT_LABELS: Record<string, string> = {
    acting: uiMessage("ui.4f91051edd00"),
    directing: uiMessage("ui.5f7970bcbecb"),
    writing: "Roteiro",
    production: uiMessage("ui.43a94f5ce3bc"),
    editing: uiMessage("ui.0c4a8a0e6f0a"),
    camera: "Fotografia",
    sound: "Som",
    art: "Arte",
    crew: uiMessage("ui.b654e894ee3c"),
    "costume & make-up": "Figurino e Maquiagem",
    "visual effects": "Efeitos visuais",
    creator: uiMessage("ui.5da53a7a65e3"),
};
...
const JOB_LABELS: Record<string, string> = {
    ...
    editor: uiMessage("ui.0c4a8a0e6f0a"),
    creator: uiMessage("ui.5da53a7a65e3"),
    novel: "Obra original",
};
...
function creditHref(credit: PoplogPersonCredit): string | null {
    ...
    const hasAnyId = Boolean(ext?.imdbId) || Boolean(ext?.traktSlug) || ext?.traktId != null || ext?.tmdbId != null;
    if (!hasAnyId)
        return null;
...
// ── CreditCard ────────────────────────────────────────────────────────────────
function CreditCard({ credit, showRole = true }: {
    credit: PoplogPersonCredit;
    showRole?: boolean;
}) {
    const href = creditHref(credit);
    const poster = resolveCatalogImage(credit.posterUrl, "w92");
    const year = creditYear(credit);
    const mediaLabel = credit.mediaType === "tv" ? uiMessage("ui.74f19285073a") : "Filme";
    const MediaIcon = credit.mediaType === "tv" ? Tv : Film;
    const role = jobLabel(credit.job);
    const inner = (<article className="group flex gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] p-2.5 transition hover:border-white/[0.12] hover:bg-white/[0.05]">
      <div className="relative flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white/[0.06]">
        {poster ? (<Image src={poster} alt={credit.title} fill sizes="56px" className="object-cover transition group-hover:scale-105"/>) : (<MediaIcon className="size-5 text-white/30"/>)}
      </div>
      <div className="min-w-0 py-0.5">
        ...
        {showRole && credit.character ? (<p className="mt-1 truncate text-[11px] text-white/35">como {credit.character}</p>) : null}
        {showRole && role && !credit.character ? (<p className="mt-1 truncate text-[11px] text-indigo-300/60">{role}</p>) : null}
      </div>
    </article>);
    if (href)
...
// ── CreditsSection ────────────────────────────────────────────────────────────
function CreditsSection({ title, icon: Icon, items, maxVisible = 30, }: {
    title: string;
    ...
    return (<section className="space-y-4">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        {shown.map((credit, i) => (<CreditCard key={`${credit.id}-${i}`} credit={credit}/>))}
      </div>
      {items.length > maxVisible ? (<p className="text-[11px] text-white/30">
          + {items.length - maxVisible}{uiMessage("ui.82c2c48b9f6e")}</p>) : null}
    </section>);
}
// ── Main ──────────────────────────────────────────────────────────────────────
export default function PersonPageClient({ person, acting, directing, writing, producing, otherCrew, }: Props) {
      ...
      <div className="flex flex-col gap-10 py-6 pb-16">
        ...
        <section className="flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-8">
          ...
          <div className="relative mx-auto size-36 shrink-0 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.04] shadow-[0_18px_50px_rgba(0,0,0,0.4)] sm:mx-0 sm:size-52">
            {profileUrl ? (<Image src={profileUrl} alt={person.name} fill sizes="(max-width: 640px) 144px, 208px" className="object-cover object-top" priority/>) : (<div className="flex h-full items-center justify-center">
          ...
          <div className="flex flex-col gap-3 sm:pt-2">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-[12px] text-white/40">
              ...
              {totalActing > 0 ? (<span className="text-white/30">
                  {totalActing}{uiMessage("ui.4ef9a15d2b9b")}{totalActing !== 1 ? uiMessage("ui.2d6ed25060b2") : uiMessage("ui.f08f8ca2d4d4")}
                  {hasCrew ? uiMessage("ui.36b45aec1e0d", { v1: totalCrew, v2: totalCrew !== 1 ? "s" : "" }) : ""}
                </span>) : hasCrew ? (<span className="text-white/30">
                  {totalCrew}{uiMessage("ui.170c477815e7")}{totalCrew !== 1 ? "s" : ""}{uiMessage("ui.8b074ab04271")}</span>) : null}
            </div>

            {person.biography ? (<p className="max-w-2xl text-[13px] leading-relaxed text-white/50 sm:text-sm sm:leading-7 line-clamp-6">
                {person.biography}
              </p>) : (<p className="text-[12px] italic text-white/30">{uiMessage("ui.d64bb1532b7a")}</p>)}
          </div>
        </section>
        ...
        <div className="flex flex-col gap-10">
          <CreditsSection title={uiMessage("ui.4f91051edd00")} icon={Clapperboard} items={acting}/>

          {acting.length > 0 && hasCrew ? (<div className="h-px bg-gradient-to-r from-transparent via-white/[0.05] to-transparent"/>) : null}

          <CreditsSection title={uiMessage("ui.5f7970bcbecb")} icon={Video} items={directing}/>
          <CreditsSection title="Roteiro" icon={PenLine} items={writing}/>
          <CreditsSection title={uiMessage("ui.43a94f5ce3bc")} icon={Star} items={producing}/>
          <CreditsSection title={uiMessage("ui.49c555ac2731")} icon={Film} items={otherCrew} maxVisible={20}/>
        </div>

        {!hasAnyCredit ? (<div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] px-6 py-10 text-center">
            <UserRound className="mx-auto mb-3 size-8 text-white/20"/>
            <p className="text-sm font-medium text-white/40">{uiMessage("ui.b991e6dc2d88")}</p>
            <p className="mt-1 text-[12px] text-white/25">{uiMessage("ui.3337877d3db4")}</p>
          </div>) : null}
