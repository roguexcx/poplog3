...
// ─── Constants ────────────────────────────────────────────────────────────────
const ALL_APIS = ["trakt", "balloonerismm", "tmdb", "watchmode", "motn", "wikidata", "justwatch"] as const;
...
const API_COLORS: Record<ApiName, string> = {
    ...
    wikidata: "text-sky-300     bg-sky-500/10     border-sky-400/20",
    justwatch: "text-yellow-300  bg-yellow-500/10  border-yellow-400/20",
};
...
function Sparkline({ values, color = "#818cf8" }: {
    ...
    return (<svg width={total} height={h} className="shrink-0 opacity-80">
      {values.map((v, i) => {
            const barH = Math.max(2, Math.round((v / max) * h));
            return (<rect key={i} x={i * (w + gap)} y={h - barH} width={w} height={barH} rx={1} fill={color} opacity={v === 0 ? 0.2 : 0.9}/>);
        })}
    </svg>);
}
// ─── Per-API summary card ─────────────────────────────────────────────────────
function ApiCard({ name, totals, sparkValues, }: {
    name: ApiName;
    totals: ApiTotals;
    sparkValues: number[];
}) {
    ...
    return (<div className={`rounded-xl border px-4 py-3 ${colors} flex flex-col gap-2`}>
      ...
      <div className="flex items-end justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5 text-[11px]">
            ...
            <span className="font-semibold">{totals.hitRate}{uiMessage("ui.0e09c4a7f690")}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[11px] opacity-70">
            <Zap size={10}/>
            <span>{totals.avgMs}{uiMessage("ui.14638ad58001")}</span>
          </div>
          {totals.errors > 0 && (<div className="text-[11px] text-red-400 font-semibold">
              {totals.errors} erros
            </div>)}
        </div>
...
// ─── Daily table ──────────────────────────────────────────────────────────────
function DailyTable({ series, activeApis }: {
    series: DayEntry[];
    activeApis: ApiName[];
}) {
      ...
      <table className="w-full text-[11px]">
        <thead>
          <tr className="border-b border-white/10">
            ...
            {activeApis.map((api) => (<th key={api} className="pb-2 px-2 text-right font-semibold text-zinc-400 min-w-[70px]">
                {api}
              </th>))}
        ...
        <tbody>
          {series.map((row) => {
            ...
            if (dayTotal === 0)
                return null;
            return (<tr key={row.day} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                ...
                {activeApis.map((api) => {
                    ...
                    return (<td key={api} className="py-1.5 px-2 text-right tabular-nums">
                      {calls > 0 ? (<span className={calls > 50 ? "text-white font-semibold" : "text-zinc-300"}>
                          {calls}
                          {a && a.errors > 0 && (<span className="ml-0.5 text-red-400 text-[10px]">⚠{a.errors}</span>)}
...
// ─── Main component ───────────────────────────────────────────────────────────
export function TabApiHistory({ secret }: {
    secret: string;
}) {
    ...
    const compact = async () => {
        if (!confirm("Compactar logs raw antigos (>90 dias) em agregados diarios?"))
            return;
    ...
    const activeApis = ALL_APIS.filter((api) => (data?.totals?.[api]?.totalCalls ?? 0) > 0);
    ...
    return (<div className="space-y-6">
      ...
      {/* Controls */}
      <AdminCard title={uiMessage("ui.c245b2cfc0ba")}>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-1">
            {WINDOWS.map((w) => (<button key={w} type="button" disabled={!isReady} onClick={() => { setDays(w); void load(w); }} className={[
          ...
          <button type="button" disabled={!isReady} onClick={() => void load(days)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-indigo-400/20 bg-indigo-500/20 px-3 text-xs font-semibold text-indigo-100 transition hover:bg-indigo-500/30 disabled:opacity-40">
            <RefreshCw size={12}/>{uiMessage("ui.66f3415f4168")}</button>

          <button type="button" disabled={!isReady} onClick={compact} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-zinc-600/40 bg-zinc-700/20 px-3 text-xs font-semibold text-zinc-300 transition hover:bg-zinc-700/30 disabled:opacity-40">
            <Archive size={12}/>{uiMessage("ui.aaefc57004b9")}</button>

          {view.kind === "ok" && (<span className="text-[10px] text-zinc-500 ml-auto">
      ...
      {/* Feedback */}
      {view.kind === "loading" && (<AdminFeedback kind="loading" message={uiMessage("ui.06316f48cd61")}/>)}
      ...
      {view.kind === "idle" && (<AdminFeedback kind="success" message={uiMessage("ui.68286a28e284")}/>)}

      {/* API Summary Cards */}
      {view.kind === "ok" && activeApis.length > 0 && (<AdminCard title={uiMessage("ui.7c40ae2cc8d2", { v1: view.days })}>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
            {activeApis.map((api) => (<ApiCard key={api} name={api} totals={data!.totals![api]!} sparkValues={sparkMap[api] ?? []}/>))}
          </div>
        </AdminCard>)}

      {/* No data */}
      {view.kind === "ok" && activeApis.length === 0 && (<AdminFeedback kind="success" message={uiMessage("ui.1f073b366ba6", { v1: view.days })}/>)}

      {/* Daily breakdown table */}
      {view.kind === "ok" && data?.series && data.series.length > 0 && activeApis.length > 0 && (<AdminCard title={uiMessage("ui.836843739108")}>
          <DailyTable series={data.series} activeApis={activeApis}/>
        </AdminCard>)}
