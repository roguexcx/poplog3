...
import { Activity, CheckCircle2, Clock, RefreshCw, RotateCcw, TrendingUp, XCircle, Zap, } from "lucide-react";
...
// ─── component ────────────────────────────────────────────────────────────────
export function TabEngineMonitor({ secret }: {
    secret: string;
}) {
    ...
    const reset = async () => {
        if (!confirm("Zerar o buffer do engine logger?"))
            return;
    ...
    useEffect(() => {
        if (autoRefresh && view.kind === "ok") {
            timerRef.current = setInterval(() => void load(view.fullMode), 10000);
        }
        else {
            if (timerRef.current)
    ...
    return (<div className="space-y-6">
      ...
      <AdminCard title="Controles">
        <div className="flex flex-wrap gap-2">
          <button type="button" disabled={!isReady} onClick={() => void load(fullMode)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-indigo-500/20 px-4 text-sm font-semibold text-indigo-100 border border-indigo-400/20 transition hover:bg-indigo-500/30 disabled:opacity-40 disabled:cursor-not-allowed">
            <Activity size={13}/>
            {view.kind === "loading" ? "Carregando…" : uiMessage("ui.66f3415f4168")}
          </button>
          ...
          {view.kind === "ok" && (<>
              ...
              <CtrlBtn onClick={() => void load(fullMode)} icon={<RefreshCw size={13}/>}>{uiMessage("ui.7ed81fa447b9")}</CtrlBtn>
              <CtrlBtn onClick={() => setAuto((v) => !v)} icon={<Zap size={13}/>} active={autoRefresh}>
                {autoRefresh ? "Auto ON" : "Auto OFF"}
              </CtrlBtn>
              <CtrlBtn onClick={() => void reset()} icon={<RotateCcw size={13}/>} tone="danger">{uiMessage("ui.22a8b6615250")}</CtrlBtn>
            </>)}
        </div>
      </AdminCard>

      {/* feedback */}
      {view.kind === "loading" && <AdminFeedback kind="loading" message={uiMessage("ui.4c7d226a65ee")}/>}
      {view.kind === "unauthorized" && <AdminFeedback kind="error" message={uiMessage("ui.0cc659bba427")}/>}
      {view.kind === "error" && <AdminFeedback kind="error" message={view.message}/>}
      {view.kind === "idle" && <p className="text-sm text-zinc-600">{uiMessage("ui.dc93b1e6f914")}</p>}
...
// ─── dashboard ────────────────────────────────────────────────────────────────
function EngineBody({ data, fetchedAt, fullMode }: {
    data: EngineData;
    fetchedAt: Date;
    fullMode: boolean;
}) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Chamadas" value={String(data.summary.totalCalls)} sub={data.summary.window ?? uiMessage("ui.726dff4f68b2")} icon={<Activity size={15} className="text-indigo-300"/>}/>
        <StatCard label={uiMessage("ui.5a11b642a82e")} value={data.summary.cacheHitRate} sub="chamadas do cache" icon={<Zap size={15} className="text-emerald-300"/>} tone={hitRate >= 70 ? "good" : hitRate >= 40 ? "neutral" : "bad"}/>
      ...
      {/* per-api */}
      <AdminCard title={uiMessage("ui.bb837cf4b105")} desc={data.summary.source === "persistent" ? uiMessage("ui.36a3cb556b08") : uiMessage("ui.d802ca9571e2")}>
        <ApiTable rows={data.perApi}/>
      </AdminCard>

      {/* per-origin */}
      <AdminCard title={uiMessage("ui.50ddbd8c5207")}>
        <OriginBars rows={data.perOrigin} total={data.summary.totalCalls}/>
      </AdminCard>

      {/* errors */}
      {data.recentErrors.length > 0 && (<AdminCard title={uiMessage("ui.aa751c0cf93b", { v1: data.recentErrors.length })} tone="danger" desc={uiMessage("ui.738130adcbc8")}>
          <LogTable entries={data.recentErrors}/>
        </AdminCard>)}

      {/* log */}
      <AdminCard title={uiMessage("ui.9cb686d9031b", { v1: data.recentCalls.length })} desc="Mais recentes em ordem decrescente.">
...
function ApiTable({ rows }: {
    ...
    if (!rows.length)
        return <p className="text-sm text-zinc-600">{uiMessage("ui.6d8348f14383")}</p>;
    return (<div className="overflow-auto rounded-lg border border-white/10">
      <table className="min-w-full text-left text-xs">
        <thead className="bg-white/[0.04] text-zinc-500">
          <tr>{["API", "Calls", "Hits", "Misses", "Erros", "Hit rate", "Avg ms", "P95 ms", "Max ms"].map((c) => (<th key={c} className="px-4 py-2.5 font-bold uppercase tracking-[0.12em]">{c}</th>))}</tr>
        </thead>
...
function OriginBars({ rows, total }: {
    ...
    if (!rows.length)
        return <p className="text-sm text-zinc-600">{uiMessage("ui.e8b82c9cdea6")}</p>;
...
function LogTable({ entries }: {
    ...
    if (!entries.length)
        return <p className="text-sm text-zinc-600">{uiMessage("ui.ba009dc9f1d5")}</p>;
    return (<div className="overflow-auto rounded-lg border border-white/10">
      <table className="min-w-full text-left text-xs">
        <thead className="bg-white/[0.04] text-zinc-500">
          <tr>{["#", "Hora", "API", "Op", "Origem", "Cache", "ms", "OK", "Endpoint", "Erro"].map((c) => (<th key={c} className="px-3 py-2.5 font-bold uppercase tracking-[0.12em]">{c}</th>))}</tr>
        </thead>
...
function CacheBadge({ status }: {
    ...
    return (<span className={`inline-flex rounded border px-1.5 py-0.5 text-[10px] font-bold ${map[status] ?? "border-white/10 text-zinc-500"}`}>
      {status}
    </span>);
}
// ─── ctrl button ──────────────────────────────────────────────────────────────
function CtrlBtn({ children, onClick, icon, active, tone }: {
    ...
    return (<button type="button" onClick={onClick} className={`inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-sm font-semibold transition ${cls}`}>
      {icon}{children}
    </button>);
}
