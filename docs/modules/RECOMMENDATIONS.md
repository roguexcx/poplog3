...
> **Revisão 2026-06-26.** Rotas atualizadas para bare `/api/*` (o discover
> personalizado é `/api/discover`; o legado `/api/poplog3/discover` não existe mais).
> Estado canônico em `../POPLOG_SYSTEM_OVERVIEW.md` e `../POPLOG_SYSTEM_DIAGNOSTIC.md`.
...
**Método:** POST com `{ titles, exclude, limit, catalogLanguage, region }`  
**Default:** 6 itens (1 featured + 5 restantes)  
**Máximo:** 30 itens

> **Nota arquitetural (2026-06-27):** esta superfície respeita a separação
> `interfaceLanguage` / `catalogLanguage` / `region` do POPLOG (ver §6 de
> `POPLOG_SYSTEM_OVERVIEW.md`). O POST **omite `interfaceLanguage`** — apenas
> `catalogLanguage` e `region` disparam recálculo de recomendação. Textos de UI
> (`reason`, `mediaLabel`) são montados na borda com o `interfaceLanguage` corrente
> via helper `src/lib/for-you/reason.ts`.

### Contrato de i18n e dados neutros

O motor produz **dados estruturais** (`ForYouReasonData`) em vez de strings finais:

| Campo no payload | Tipo | Semântica |
|---|---|---|
| `reasonCode` | `ForYouReasonCode` | Código neutro da razão (`because_favorited`, `from_watchlist`, etc.) |
| `reasonSeedTitle` | `string \| null` | Título da semente — metadado de catálogo, já no `catalogLanguage` |
| `reasonMoreCount` | `number` | Qtd de outros títulos para o sufixo "+N outros" |
| `reasonGenrePrefix` | `string \| null` | Prefixo de gênero (catálogo), prepended à frase |
| `reason` | `string` | ⚠️ Back-compat: frase pré-montada no `interfaceLanguage` do request |
| `mediaLabel` | `string` | ⚠️ Back-compat: label de mídia pré-montada no request |

Consumidores **novos** usam `reasonCode` + helpers `renderForYouReason()` /
`renderForYouMediaLabel()` de `src/lib/for-you/reason.ts`.  
Consumidores **legados** continuam usando `reason` / `mediaLabel` durante a transição.

**Helper `src/lib/for-you/reason.ts`:**

```ts
// Tipos
type ForYouReasonCode = "because_favorited" | "because_watching" | "based_on_watched"
                      | "from_watchlist"    | "based_on_library" | "library_match";
type ForYouReasonData = {
  code: ForYouReasonCode;
  seedTitle?:    string | null;  // catalogLanguage
  moreCount?:    number;
  genrePrefix?:  string | null;  // catalogLanguage
};

// Renderers (recebem ui() ou uiMessageFor() como tradutor)
renderForYouReason(data, translate)      // → frase completa no interfaceLanguage
renderForYouMediaLabel(mediaType, translate)  // → "Filme" | "Série"
```

Chaves i18n usadas: `for_you.reason.{code}`, `for_you.reason.more_suffix_one/other`,
`for_you.reason.fallback_title`, `for_you.media.movie`, `for_you.media.tv`.

### Cache do pool — sem contaminação por `interfaceLanguage`

**Chave do pool (in-memory + persistido):**
```
pool:v4:{catalogLanguage}:{region}:{libraryHash}
```

`interfaceLanguage` **não faz parte da chave** — o pool guarda `_reasonData` neutro,
não strings traduzidas. Trocar o idioma da interface nunca invalida nem contamina o
pool cacheado.

> **Bug corrigido (2026-06-27):** antes da refatoração, o pool podia guardar `reason`
> já traduzida e servir texto em pt-BR para um usuário que tinha trocado para en-US
> (cache-hit de sessão diferente compartilhando chave sem `interfaceLanguage`).
...
3. Itens resultantes viram `RecCandidate[]` com `seedEffectiveWeight`, `relationStrength` (0–100)
   e `_reasonData: ForYouReasonData` — dados estruturais neutros da explicação (sem tradução)
...
**Campos de razão no payload:**  
`buildForYouResponse` emite tanto os campos neutros (`reasonCode`, `reasonSeedTitle`,
`reasonMoreCount`, `reasonGenrePrefix`) quanto os campos de back-compat (`reason`,
`mediaLabel`) montados via `renderReasonString(data, interfaceLanguage)` no momento
da resposta HTTP — não no pool cacheado.

### Consumidores do payload

| Componente | Arquivo | Comportamento |
|---|---|---|
| Home | `src/features/home/ForYouSection.tsx` | POST com `catalogLanguage`+`region`; omite `interfaceLanguage`; renderiza via `renderForYouReason(item._reasonData ?? {code,...}, ui)` com fallback para `item.reason` legado |
| Para Você full | `src/features/for-you/ForYouAllPage.tsx` | Mesmas regras; reset de paginação/histórico quando `catalogLanguage`/`region` mudam |
| `not-interested/route.ts` | — | Usa campo `reason` de DB — **não** relacionado a este contrato |

`interfaceLanguage` **não dispara refetch pesado** em nenhuma das duas superfícies.
Trocar o idioma de interface re-renderiza o texto no client sem nova chamada à API.
...
- **Separação i18n no "Para Você" (2026-06-27):** motor produz dados neutros (`ForYouReasonData`);
  texto de UI montado na borda com `interfaceLanguage` corrente; pool cacheado sem contaminação
  por idioma de interface; `ForYouSection`/`ForYouAllPage` omitem `interfaceLanguage` do fetch pesado
...
- **Discovery `/api/discover` não tem personalização** — não aplica feedback do usuário.
