...
>
> **Revisão 2026-06-26 (feature/sistema-novo).** Este mapa é de 24/06 e tem itens
> superados; o estado canônico está em `../POPLOG_SYSTEM_OVERVIEW.md` §8 e
> `../POPLOG_SYSTEM_DIAGNOSTIC.md` §3. Correções: o banco é **MySQL** (não PostgreSQL);
> os clients **TheTVDB e OMDb foram REMOVIDOS** — episódios vêm de Trakt+Balloonerismm
> e `tvdbId`/`still_source="tvdb"` permanecem só como alias/histórico; as rotas
> `/api/poplog3/*` foram migradas para **bare `/api/*`**; o módulo **Agenda foi removido**
> (as linhas de TheTVDB/OMDb na tabela §1.1 abaixo são históricas).
...
- **MySQL via Prisma** (`prisma/schema.prisma`, `src/server/db/client.ts`) —
...
Legenda de fluxo: `→` ordem de fallback · **DB** = MySQL local.
...
### Acompanhando (`/api/acompanhando`, `continuity/`)
